...
const cloneDeep = require('../utils/clone');
...
client.load = function load (serverSettings, callback) {
   ...
   * @returns {boolean}
   */
  client.isAuthorized = function isAuthorized (permission) {
    if (client.authorized && client.authorized.shiros) { // Ensure client.authorized.shiros exists
      var found = client.authorized.shiros.find(function checkEach (shiro) {
  ...
  function brushed () {
    ...
    function updatePlugins (time) {
      ...
      var clonedData = cloneDeep(client.ddata);
      ...
      client.sbx.pluginBase.forecastInfos?.forEach(function eachInfo (info) {
        ...
        forecastCheckbox.change(function onChange (event) {
          ...
          if (checked) {
            ...
            client.settings.showForecast = client.settings.showForecast.split(' ')
  ...
  function generateAlarm (file, notify) {
    ...
    client.settings.snoozeMinsForAlarmEvent(notify)?.forEach(function eachOption (mins) {
