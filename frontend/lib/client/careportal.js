...
function init (client, $) {
  ...
  function setDateAndTime (time) {
    time = time || client.ctx.dayjs() || client.ctx.moment();
  ...
  function refreshEventTypes() {
    ...
    careportal.events = careportal.allEventTypes.map(function each (event) {
      return pick(event, ['val', 'name']);
    });

    // Initialize the input matrix first to ensure safe access later
    var newInputMatrix = {};
    submitHooks = {};
    careportal.allEventTypes.forEach(function each (event) {
      if (event && event.val) {
        newInputMatrix[event.val] = pick(event, ['otp','remoteCarbs', 'remoteAbsorption', 'remoteBolus', 'bg', 'insulin', 'carbs', 'protein', 'fat', 'prebolus', 'duration', 'percent', 'absolute', 'profile', 'split', 'sensor', 'reasons', 'targets']);
        submitHooks[event.val] = event.submitHook;
      }
    });
    // Only assign after fully populated
  ...
  careportal.resolveEventName = function resolveEventName (value) {
    careportal.events?.forEach(function eachEvent (e) {
      if (e.val === value) {
        value = e.name;
      }
    });
  ...
  function gatherData () {
    ...
    var reason = reasons && reasons.length > 0 ? reasons.find(function matches (r) {
