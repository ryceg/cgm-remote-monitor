...
function init (client, $) {
  ...
  boluscalc.gatherBoluscalcData = function gatherBoluscalcData () {
    ...
    // Load foods
    record.carbs = 0;
    record.foods = JSON.parse(JSON.stringify(foods));
    if (record.foods.length) {
      var gisum = 0;
      for (var fi = 0; fi < record.foods.length; fi++) {
        /* eslint-disable-next-line security/detect-object-injection */ // verified false positive
  ...
  function quickpickChange (event) {
    ...
    if (qpiselected === null || qpiselected === '-1') { // (none)
      ...
      foods = JSON.parse(JSON.stringify(qp.foods));
  ...
  /**
   * Find the closest SGV entry before the specified time
   * @param {Date} time - The time to find the closest entry for
   * @returns {Object|null} - The closest SGV entry or null if none found
   */
