...
var init = function init() {
  ...
  client.init(function loaded() {
    ...
    peStatus
      .hide()
      .text(translate("Loading profile records ..."))
      .fadeIn("slow");
    $.ajax("/api/v1/profile.json?count=20", {
      ...
      success: function (records) {
        ...
        if (mongorecords.length) {
          _.each(mongorecords, function eachMongoProfile(mongoprofile) {
            _.each(mongoprofile.store, function eachStoredProfile(p) {
              ...
              for (var key in defaultprofile) {
                if (
                  Object.prototype.hasOwnProperty.call(defaultprofile, key) &&
          ...
          peStatus
            .hide()
            .text(translate("Default values used."))
            .fadeIn("slow");
        }
      },
      error: function () {
        ...
        peStatus
          .hide()
          .text(translate("Error. Default values used."))
          .fadeIn("slow");
      },
    }).done(initeditor);
    ...
    function convertToRanges(profile) {
      if (typeof profile.carbratio !== "object") {
        profile.carbratio = [{ time: "00:00", value: profile.carbratio }];
      }
      if (typeof profile.sens !== "object") {
        profile.sens = [{ time: "00:00", value: profile.sens }];
      }
      if (typeof profile.target_low !== "object") {
        profile.target_low = [{ time: "00:00", value: profile.target_low }];
      }
      if (typeof profile.target_high !== "object") {
        profile.target_high = [{ time: "00:00", value: profile.target_high }];
      }
      if (typeof profile.basal !== "object") {
        profile.basal = [{ time: "00:00", value: profile.basal }];
      }
    ...
    function initRecord() {
      ...
      timeInput.val(
        client.ctx
          .dayjs(mongorecords[currentrecord].startDate)
          .format("HH:mm"),
      );
      dateInput.val(
        client.ctx
    ...
    function recordRemove(event) {
      if (
        mongorecords.length > 1 &&
        window.confirm(translate("Delete record") + "?")
      ) {
    ...
    function recordClone(event) {
      ...
      GUIToObject();
      mongorecords.push(
        _.omit(mongorecords[currentrecord], [
          "_id",
          "srvModified",
          "srvCreated",
          "identifier",
    ...
    function profileChange(event) {
      ...
      if (newpr === currentprofile) {
        // fake call to update values
        newpr = newname;
      }
    ...
    function fillTimeRanges(event) {
      ...
      function addSingleLine(e, i) {
        var tr = $("<tr>");
        var select = $("<select>")
          .attr("class", "pe_selectabletime")
          .attr("id", e.prefix + "_from_" + i);
        var lowest = -1;
        for (var t = 0; t < 48; t++) {
        ...
        var selectedValue =
          toMinutesFromMidnight(c_profile[e.array][i].time) * 30;
      ...
      _.each(
        ...
        function (e) {
          ...
          html +=
            '<tr><td></td><td></td><td><img class="addsingle" style="cursor:pointer" title="' +
      ...
      function addBGLine(i) {
        ...
        var selectedValue =
          toMinutesFromMidnight(c_profile.target_low[i].time) * 30;
        select.val(selectedValue);
        tr.append(
          $("<td>")
            .append(translate("From") + ": ")
      ...
      $(".pe_selectabletime")
        .unbind()
        .on("change", fillTimeRanges)
        .on("change", refreshTotalBasal);
    ...
    function GUIToObject() {
      ...
      if (c_profile.timezone.toLowerCase() == timezoneInput.val().toLowerCase())
        zone = c_profile.timezone;
    ...
    function toTimeString(minfrommidnight) {
      return client.ctx.dayjs
        .utc()
        .startOf("day")
        .add(minfrommidnight, "minutes")
        .format("HH:mm"); // using utc to avoid daylight saving offset
    }

    function toDisplayTime(minfrommidnight) {
      var time = client.ctx.dayjs
        .utc()
        .startOf("day")
        .add(minfrommidnight, "minutes"); // using utc to avoid daylight saving offset
