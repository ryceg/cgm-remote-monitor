...
//  defaultprofile.startDate.setMilliseconds(0);
  ...
  $.ajax('/api/v1/profile.json?count=20', {
    ...
    , success: function (records) {
      ...
      mongorecords = structuredClone(client.profilefunctions.data);
      // create new profile to be edited from last record
      if (mongorecords.length) {
        mongorecords.forEach(function eachMongoProfile (mongoprofile) {
          Object.values(mongoprofile.store || {}).forEach(function eachStoredProfile (p) {
        ...
        mongorecords.push({
          ...
          , store : {
            'Default': structuredClone(defaultprofile)
          }
  ...
  function convertToRanges(profile) {
    ...
    if (profile.target_high.length !== profile.target_low.length) {
      ...
      profile.target_low = structuredClone(defaultprofile.target_low);
      profile.target_high = structuredClone(defaultprofile.target_high);
    }
  }
  ...
  function initRecord() {
    ...
    timeInput.val(client.ctx.dayjs(mongorecords[currentrecord].startDate).format('HH:mm'));
    dateInput.val(client.ctx.dayjs(mongorecords[currentrecord].startDate).format('YYYY-MM-DD'));
  ...
  function recordClone (event) {
    ...
    const { _id, srvModified, srvCreated, identifier, mills, ...recordWithoutMetadata } = mongorecords[currentrecord];
    mongorecords.push(recordWithoutMetadata);
    currentrecord = mongorecords.length - 1;
  ...
  function profileAdd (event) {
    ...
    record.store[newname] = structuredClone(defaultprofile);
  ...
  function profileClone (event) {
    ...
    record.store[newname] = structuredClone(record.store[currentprofile]);
  ...
  function fillTimeRanges(event) {
    ...
    [{prefix:'pe_basal', array:'basal', label: translate('Basal rate') + ' : '},
  ...
  function GUIToObject() {
    var oldProfile = structuredClone(c_profile);
    ...
    if (JSON.stringify(oldProfile) !== JSON.stringify(c_profile)) {
      dirty = true;
    }
  }
  /**
  ...
  function toTimeString(minfrommidnight) {
    return client.ctx.dayjs.utc().startOf('day').add(minfrommidnight,'minutes').format('HH:mm'); // using utc to avoid daylight saving offset
  }

  function toDisplayTime (minfrommidnight) {
    var time = client.ctx.dayjs.utc().startOf('day').add(minfrommidnight,'minutes'); // using utc to avoid daylight saving offset
  ...
  function profileSubmit(event) {
    ...
    var adjustedRecord = structuredClone(record);
