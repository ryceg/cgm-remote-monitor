  ...
  client.init(function loaded() {
    ...
    function doFoodFilter(event) {
      ...
      for (var i = 0; i < food_list.length; i++) {
        if (
          filter.category !== "" &&
          food_list[i].category !== filter.category
        ) {
          continue;
        }
        if (
          filter.subcategory !== "" &&
          food_list[i].subcategory !== filter.subcategory
        ) {
          continue;
        }
        if (
          filter.name !== "" &&
          food_list[i].name.toLowerCase().indexOf(filter.name.toLowerCase()) < 0
        ) {
          continue;
        }
        var o = "";
    ...
    $.ajax("/api/v1/food/regular.json", {
      ...
      success: function foodLoadSuccess(records) {
        records.forEach(function (r) {
          food_list.push(r);
          if (r.category && !food_categories[r.category]) {
            food_categories[r.category] = {};
          }
          if (r.category && r.subcategory) {
            food_categories[r.category][r.subcategory] = true;
          }
        });
        fillFoodForm();
      },
    ...
    function show(event) {
      ...
      var timerange =
        "&find[created_at][$gte]=" +
        dayjs.tz("2000-01-01", zone).toISOString();
      ...
      options.scale = $("#rp_linear").is(":checked")
        ? report_plugins.consts.SCALE_LINEAR
        : report_plugins.consts.SCALE_LOG;
      options.weekscale = $("#wrp_linear").is(":checked")
        ? report_plugins.consts.SCALE_LINEAR
        : report_plugins.consts.SCALE_LOG;
      options.order = $("#rp_oldestontop").is(":checked")
        ? report_plugins.consts.ORDER_OLDESTONTOP
        : report_plugins.consts.ORDER_NEWESTONTOP;
      ...
      function datefilter() {
        if ($("#rp_enabledate").is(":checked")) {
          matchesneeded++;
          var from = dayjs
            .tz(dayjs($("#rp_from").val()).startOf("day"), zone)
            .startOf("day");
          var to = dayjs
            .tz(dayjs($("#rp_to").val()).endOf("day"), zone)
            .endOf("day");
          timerange =
            "&find[created_at][$gte]=" +
            from.toISOString() +
      ...
      function daysfilter() {
        ...
        Object.keys(daystoshow).forEach(function eachDay(d) {
          var day = dayjs.tz(d, zone).day();
          if (day === 0 && $("#rp_su").is(":checked")) {
            daystoshow[d]++;
          }
          if (day === 1 && $("#rp_mo").is(":checked")) {
            daystoshow[d]++;
          }
          if (day === 2 && $("#rp_tu").is(":checked")) {
            daystoshow[d]++;
          }
          if (day === 3 && $("#rp_we").is(":checked")) {
            daystoshow[d]++;
          }
          if (day === 4 && $("#rp_th").is(":checked")) {
            daystoshow[d]++;
          }
          if (day === 5 && $("#rp_fr").is(":checked")) {
            daystoshow[d]++;
          }
          if (day === 6 && $("#rp_sa").is(":checked")) {
            daystoshow[d]++;
          }
        });
        countDays();
    ...
    function showreports(options) {
      ...
      report_plugins.eachPlugin(function (plugin) {
        ...
        if (plugin.name == "daytoday" && !$("#daytoday").hasClass("selected"))
          skipRender = true;
        if (
          plugin.name == "treatments" &&
          !$("#treatments").hasClass("selected")
        )
          skipRender = true;
        if (
          plugin.name == "weektoweek" &&
          !$("#weektoweek").hasClass("selected")
        )
          skipRender = true;
        if (
          plugin.name == "loopalyzer" &&
          !$("#loopalyzer").hasClass("selected")
        )
          skipRender = true;
    ...
    function setDataRange(event, days) {
      $("#rp_to").val(dayjs().format("YYYY-MM-DD"));
      $("#rp_from").val(
        dayjs()
          .add(-days + 1, "days")
          .format("YYYY-MM-DD"),
    ...
    function loadData(day, options, callback) {
      ...
      if (
        (options.openAps || options.predicted || options.iob || options.cob) &&
        datastorage[day] &&
        !datastorage[day].devicestatus.length
      ) {
        // OpenAPS requested but data not loaded. Load anyway ...
      } else if (datastorage[day] && day !== dayjs().format("YYYY-MM-DD")) {
        callback(day);
        return;
      }
      ...
      if (client.sbx.data.profile.getTimezone()) {
        from = dayjs(day)
          .tz(client.sbx.data.profile.getTimezone())
          .startOf("day")
          .format("x");
      } else {
        from = dayjs(day).startOf("day").format("x");
      }
      from = parseInt(from);
        ...
        return $.ajax("/api/v1/entries.json" + query, {
          ...
          success: function (xhr) {
            ...
            data.sgv = data.sgv.filter(function (d) {
              ...
              lastDate = d.mills;
              if (!ok) {
                console.log("itm", JSON.stringify(d));
              }
              return ok;
            });
            data.mbg = mbgData.slice();
            data.mbg.sort(function (a, b) {
              return a.mills - b.mills;
            });
            data.cal = calData.slice();
            data.cal.sort(function (a, b) {
              return a.mills - b.mills;
            });
          },
        });
      ...
      $.ajax("/api/v1/treatments.json" + tquery, {
        ...
        success: function (xhr) {
          ...
          if (!datastorage.profileSwitchTreatments)
            datastorage.profileSwitchTreatments = [];
          datastorage.profileSwitchTreatments =
            datastorage.profileSwitchTreatments.concat(treatmentData);
          datastorage.profileSwitchTreatments.sort(function (a, b) {
            return a.mills - b.mills;
          });
        },
      }).done(function () {
    ...
    function processData(data, day, options, callback) {
      ...
      if (cal) {
        temp1 = data.sgv
          .map(function (entry) {
            ...
            return {
              mills: entry.mills,
              date: new Date(entry.mills - 2 * 1000),
              y: rawBg,
              sgv: client.utils.scaleMgdl(rawBg),
              color: "gray",
              type: "rawbg",
      ...
      var temp2 = data.sgv.map(function (obj) {
        return {
          mills: obj.mills,
          date: new Date(obj.mills),
          y: obj.y,
          sgv: client.utils.scaleMgdl(obj.y),
          color: sgvToColor(client.utils.scaleMgdl(obj.y), options),
          type: "sgv",
          noise: obj.noise,
      ...
      data.sgv = data.sgv.concat(
        data.mbg.map(function (obj) {
          return {
            date: new Date(obj.mills),
            y: obj.y,
            sgv: client.utils.scaleMgdl(obj.y),
