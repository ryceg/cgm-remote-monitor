...
var init = function init () {
  ...
  client.init(function loaded () {
    ...
    function show (event) {
      ...
      var timerange = '&find[created_at][$gte]=' + dayjs.tz('2000-01-01', zone).toISOString();
      ...
      function datefilter () {
        if ($('#rp_enabledate').is(':checked')) {
          matchesneeded++;
          var from = dayjs($('#rp_from').val()).startOf('day');
          var to = dayjs($('#rp_to').val()).endOf('day');
          timerange = '&find[created_at][$gte]=' + from.tz(zone).toISOString() + '&find[created_at][$lt]=' + to.tz(zone).toISOString();
          console.log("FROM", from.format('YYYY-MM-DDTHH:mm:ss.SSSZ'), "TO", to.format('YYYY-MM-DDTHH:mm:ss.SSSZ'));
          console.log('timerange', timerange);
          //console.log($('#rp_from').val(),$('#rp_to').val(),zone,timerange);
          while (from.unix() <= to.unix()) {
            const dateKey = from.format('YYYY-MM-DD');
            if (daystoshow[dateKey]) {
              daystoshow[dateKey]++;
            } else {
              daystoshow[dateKey] = 1;
            }
            ...
            $.ajax('/api/v1/treatments.json' + tquery, {
              ...
              , success: function(xhr) {
                treatmentData = xhr.map(function(treatment) {
                  return dayjs.tz(treatment.created_at, zone).format('YYYY-MM-DD');
                });
                // unique it
      ...
      function dataLoadedCallback (day) {
        ...
        if (loadeddays === dayscount) {
          ...
          var dFrom = dayjs(sorteddaystoshow[0]);
          var dTo = dayjs(sorteddaystoshow[(sorteddaystoshow.length - 1)]);
    ...
    function setDataRange (event, days) {
      ...
      $('#rp_from').val(dayjs().add(-days + 1, 'days').format('YYYY-MM-DD'));
      return maybePrevent(event);
    }
    ...
    function loadData (day, options, callback) {
      ...
      if (client.sbx.data.profile.getTimezone()) {
        from = dayjs(day).startOf('day').tz(client.sbx.data.profile.getTimezone());
      } else {
        from = dayjs(day).startOf('day')
      }
      var to = dayjs(from).add(1, 'days');
    ...
    function processData (data, day, options, callback) {
      ...
      var from;
      if (client.sbx.data.profile.getTimezone()) {
        from = dayjs(day).tz(client.sbx.data.profile.getTimezone()).startOf('day').toDate();
      } else {
