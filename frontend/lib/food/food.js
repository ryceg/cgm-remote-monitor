...
var init = function init() {
  ...
  client.init(function loaded() {
    ...
    var foodrec = JSON.parse(JSON.stringify(foodrec_template));
    ...
    $.ajax("/api/v1/food.json", {
      ...
      success: function ajaxSuccess(records) {
        ...
        $("#fe_status")
          .hide()
          .text(translate("Database loaded"))
          .fadeIn("slow");
        foodquickpick.sort(function compare(a, b) {
          return cmp(parseInt(a.position), parseInt(b.position));
        });
      },
      error: function ajaxError() {
        $("#fe_status")
          .hide()
          .text(translate("Error: Database failed to load"))
          .fadeIn("slow");
      },
    }).done(initeditor);
    ...
    function fillFilterSubcategories(event) {
      ...
      filter.subcategory = "";
      $("#fe_filter_subcategory")
        .empty()
        .append(new Option(translate("(none)"), ""));
      if (filter.category !== "") {
        for (s in categories[filter.category]) {
          if (
            Object.prototype.hasOwnProperty.call(categories[filter.category], s)
          ) {
    ...
    function doEdit(event) {
      ...
      foodrec = JSON.parse(JSON.stringify(foodlist[index]));
      objectToGUI();
      updateSaveButton();
      maybePreventDefault(event);
      return false;
    ...
    function updateFoodArray(newrec) {
      for (var i = 0; i < foodlist.length; i++) {
        if (foodlist[i]._id === newrec._id) {
          foodlist[i] = JSON.parse(JSON.stringify(newrec));
    ...
    function doFilter(event) {
      ...
      $("#fe_data_header")
        ...
        .append(
          $("<span>").attr("class", "width200px").append(translate("Name")),
        )
        .append(
          $("<span>")
            .attr("class", "width150px")
            .css("text-align", "center")
            .append(translate("Portion")),
        )
        .append(
          $("<span>")
            .attr("class", "width50px")
            .css("text-align", "center")
            .append(translate("Unit")),
        )
        .append(
          $("<span>")
            .attr("class", "width100px")
            .css("text-align", "center")
            .append(translate("Carbs")),
        )
        .append(
          $("<span>")
            .attr("class", "width100px")
            .css("text-align", "center")
            .append(translate("GI") + " [1-3]"),
        )
        .append(
          $("<span>").attr("class", "width150px").append(translate("Category")),
        )
        .append(
          $("<span>")
            .attr("class", "width150px")
            .append(translate("Subcategory")),
        )
        .append(
          $("<span>").attr("class", "width100px").append(translate("Fat [g]")),
        )
        .append(
          $("<span>")
            .attr("class", "width100px")
            .append(translate("Protein [g]")),
        )
        .append(
          $("<span>")
            .attr("class", "width100px")
            .append(translate("Energy [kJ]")),
        );
      ...
      for (var i = 0; i < foodlist.length; i++) {
        if (
          filter.category !== "" &&
          foodlist[i].category !== filter.category
        ) {
          continue;
        }
        if (
          filter.subcategory !== "" &&
          foodlist[i].subcategory !== filter.subcategory
        ) {
          continue;
        }
        if (
          filter.name !== "" &&
          foodlist[i].name.toLowerCase().indexOf(filter.name.toLowerCase()) < 0
        ) {
          continue;
        }
          ...
          $("<div>")
            ...
            .append(
              $("<span>")
                .addClass("width150px")
                .css("text-align", "center")
                .append(foodlist[i].portion),
            )
            .append(
              $("<span>")
                .addClass("width50px")
                .css("text-align", "center")
                .text(foodlist[i].unit),
            )
            .append(
              $("<span>")
                .addClass("width100px")
                .css("text-align", "center")
                .append(foodlist[i].carbs),
            )
            .append(
              $("<span>")
                .addClass("width100px")
                .css("text-align", "center")
                .append(foodlist[i].gi),
            )
            .append(
              $("<span>").addClass("width150px").text(foodlist[i].category),
            )
            .append(
              $("<span>").addClass("width150px").text(foodlist[i].subcategory),
            )
            .append($("<span>").addClass("width100px").append(foodlist[i].fat))
            .append(
              $("<span>").addClass("width100px").append(foodlist[i].protein),
            )
            .append(
              $("<span>").addClass("width100px").append(foodlist[i].energy),
            ),
        );
      }
    ...
    function drawQuickpick() {
      function addHeaderOrHint(q, i) {
        if (q.foods.length) {
          $("#fe_qpfieldset_" + i)
            ...
            .append(
              $("<span>").addClass("width200px").append(translate("Name")),
            )
            .append(
              $("<span>")
                .addClass("width150px")
          ...
          $("#fe_qpfieldset_" + i).append(
            $("<i>").append("-&gt; " + translate("Drag&drop food here")),
          );
        }
      }

      function addHiddenCount(hiddentotal) {
        $("#fe_quickpick_hiddencount").text(
      ...
      for (var i = 0; i < foodquickpick.length; i++) {
        ...
        if (showhidden === false && q.hidden) {
          hiddentotal++;
          continue;
        }
        $("#fe_picklist").append(
          $("<fieldset>")
            .attr("id", "fe_qpfieldset_" + i)
            .addClass("sortablequickpick")
            .attr("index", i)
            .attr("_id", q._id)
            .append(
              $("<legend>")
                .append(
                  $("<img>")
                    .attr("title", translate("Move to the top"))
                    .attr("src", icon_up)
                    .attr("index", i)
                    .addClass("fe_qpupimg"),
                )
                ...
                .append(
                  $('<input type="checkbox">')
                    .addClass("fq_hidden")
                    .attr("index", i)
                    .prop("checked", q.hidden),
                )
                .append(translate("Hidden"))
                .append(
                  $('<input type="checkbox">')
                    .addClass("fq_hideafteruse")
                    .attr("index", i)
                    .prop("checked", q.hideafteruse),
                )
                .append(translate("Hide after use"))
                .append(
                  " | " + translate("Carbs") + ": " + q.carbs.toFixed(0) + " g",
                ),
            ),
        );
        ...
        for (var j = 0; j < q.foods.length; j++) {
          ...
          $("#fe_qpfieldset_" + i).append(
            $("<div>")
              .attr("id", "fqp_food_" + i + "_" + j)
              .addClass("fe_foodinsideqp")
              .append(
                $("<span>")
                  .addClass("width50px")
                  .append(
              ...
              .append(
                $('<input type="text">')
                  .attr("id", "fq_portions_" + q._id + "_" + j)
                  .attr("index", i)
                  .attr("findex", j)
                  .attr("value", r.portions)
                  .addClass("fe_qpportions"),
              ),
          );
        }
      }
    ...
    function clearRec(event) {
      foodrec = JSON.parse(JSON.stringify(foodrec_template));
      objectToGUI();
      updateSaveButton();
      maybePreventDefault(event);
      return false;
    ...
    function foodSubmit(event) {
      ...
      if (
        $("#fe_editcreate").text().indexOf(translate("Create new record")) > -1
      ) {
    ...
    function quickpickCreateRecord(event) {
      var newrec = JSON.parse(JSON.stringify(quickpickrec_template));
