...
function init (ctx) {
  ...
  pump.getPrefs = function getPrefs (sbx) {
    ...
    function isEmpty (list) {
      return !list || list.length === 0 || !list[0] || list[0].length === 0;
    }
  ...
  pump.setProperties = function setProperties (sbx) {
    sbx.offerProperty('pump', function setPump ( ) {
      ...
      var filtered = sbx.data.devicestatus.filter(function (status) {
        return ('pump' in status) && sbx.entryMills(status) <= sbx.time && sbx.entryMills(status) >= recentMills;
      });

      var pumpStatus = null;
      filtered.forEach(function each (status) {
        status.clockMills = status.pump && status.pump.clock ? dayjs(status.pump.clock).valueOf() : status.mills;
  ...
  pump.updateVisualisation = function updateVisualisation (sbx) {
    ...
    var selectedFields = sbx.data.inRetroMode ? prefs.retroFields : prefs.fields;    ALL_STATUS_FIELDS.forEach(function eachField (fieldName) {
      var field = result[fieldName];
      if (field) {
        var selected = selectedFields.includes(fieldName);
        if (selected) {
    ...
    if (result.extended) {
      ...
      Object.entries(result.extended).forEach(([key, value]) => {
  ...
  function virtAsstReservoirHandler (next, slots, sbx) {
    var reservoir = sbx && sbx.properties && sbx.properties.pump && sbx.properties.pump.pump ? sbx.properties.pump.pump.reservoir : undefined;
  ...
  function virtAsstBatteryHandler (next, slots, sbx) {
    var battery = sbx?.properties?.pump?.data?.battery;
    if (battery) {
  ...
  function updateClock (prefs, result, sbx) {
    if (result.clock) {
      ...
      var urgent = dayjs(sbx.time).subtract(prefs.urgentClock, 'minutes');
      var warn = dayjs(sbx.time).subtract(prefs.warnClock, 'minutes');
  ...
  function prepareData (prop, prefs, sbx) {
    ...
    var time = (sbx.data.profile && sbx.data.profile.getTimezone()) ? dayjs(sbx.time).tz(sbx.data.profile.getTimezone()) : dayjs(sbx.time);
    var now = time.hour() + time.minute() / 60.0 + time.second() / 3600.0;
    var batteryWarn = !(prefs.warnBattQuietNight && (now < prefs.dayStart || now > prefs.dayEnd));
    var result = {
      level: levels.NONE
      , clock: pump.clock ? { value: dayjs(pump.clock) } : null
    ...
    if (openaps.findOfflineMarker(sbx)) {
      console.info('OpenAPS known offline, not checking for alerts');    } else {
      ALL_STATUS_FIELDS.forEach(function eachField(fieldName) {
        var field = result[fieldName];
