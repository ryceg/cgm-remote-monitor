...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  function drawChart (day, data, options) {
    ...
    charts = d3.select('#daytodaychart-' + day).html(
      '<b>' +
      report_plugins.utils.localeDate(dayjs(day)) +
      '</b><br>'
    ).append('svg');
    ...
    iobpolyline += (xScale2(dayjs(from)) + padding.left) + ',' + (yInsulinScale(0) + padding.top) + ' ';
    cobpolyline += (xScale2(dayjs(from)) + padding.left) + ',' + (yCarbsScale(0) + padding.top) + ' ';
    ...
    for (var dt = dayjs(from); dt.unix() < to.unix(); dt = dt.add(5, 'minutes')) {
