...
daytoday.report = function report_daytoday(
  ...
  function drawChart(day, data, options) {
    ...
    tickValues = client.ticks(client, {
      scaleY:
        options.scale === report_plugins.consts.SCALE_LOG ? "log" : "linear",
    ...
    xScale2 = d3.scaleTime().domain(d3.extent(data.sgv, dateFn));

    if (options.scale === report_plugins.consts.SCALE_LOG) {
      yScale2 = d3
        .scaleLog()
        .domain([
          client.utils.scaleMgdl(options.basal ? 30 : 36),
          client.utils.scaleMgdl(420),
        ]);
    } else {
      yScale2 = d3
        .scaleLinear()
        .domain([
          client.utils.scaleMgdl(options.basal ? -40 : 36),
          client.utils.scaleMgdl(420),
        ]);
    }
    ...
    yCarbsScale = d3.scaleLinear().domain([0, options.maxCarbsValue * 1.25]);

    yScaleBasals = d3.scaleLinear();

    xAxis2 = d3.axisBottom(xScale2).tickFormat(timeTicks).ticks(24);
    ...
    charts.attr("width", options.width).attr("height", options.height);
    ...
    function prepareContextCircles(sel) {
      ...
      sel
        ...
        .attr("stroke-width", function (d) {
          if (d.type === "mbg") {
            return 2;
          } else if (options.openAps && d.openaps) {
            return 1;
          } else {
            return 0;
          }
    ...
    function preparePredictedData() {
      ...
      for (var i in data.treatments) {
        ...
        if (
          undefined != treatment.carbs &&
          null != treatment.carbs &&
          treatment.carbs > 0
        ) {
      ...
      if (data && data.devicestatus) {
        for (i = data.devicestatus.length - 1; i >= 0; i--) {
          if (
            data.devicestatus[i].loop &&
            data.devicestatus[i].loop.predicted
          ) {
            predictions.push(data.devicestatus[i].loop.predicted);
          } else if (
            data.devicestatus[i].openaps &&
      ...
      if (predictions.length > 0 && treatmentsTimestamps.length > 0) {
        ...
        for (
          ...
          treatmentsIndex < treatmentsTimestamps.length;
          treatmentsIndex++
        ) {
    ...
    function findPredicted(predictions, timestamp, offset) {
      ...
      if (offset && offset < 0) {
        ...
        // If offset is positive or zero, start searching from last prediction going backward
    ...
    var contextData = options.predicted
      ? data.sgv.concat(preparePredictedData())
      : data.sgv;
    ...
    if (options.basal) {
      ...
      var basalMax = d3.max(linedata, function (d) {
        return d.b;
      });
      basalMax = Math.max(
        basalMax,
      ...
      var valueline = d3
        ...
        .x(function (d) {
          return xScale2(d.d) + padding.left;
        })
        .y(function (d) {
          return yScaleBasals(d.b) + padding.top;
        });
      ...
      datastorage.tempbasalTreatments.forEach(function (t) {
        ...
        if (
          t.mills < to.format("x") &&
          t.mills + times.mins(t.duration).msecs > from.format("x")
        ) {
          var text = g
            .append("text")
          ...
          if (
            ...
            xScale2(t.mills + times.mins(t.duration).msecs) - xScale2(t.mills)
          ) {
    ...
    data.treatments.forEach(function (treatment) {
      ...
      if (
        (treatment.boluscalc &&
          treatment.boluscalc.foods &&
          treatment.boluscalc.foods.length > 0) ||
        renderNotes
      ) {
      ...
      if (
        ...
        treatment.eventType === "Temporary Override" &&
        treatment.duration
      ) {
    ...
    if (options.insulindistribution) {
      ...
      var chartData = [
        ...
        {
          label: translate("Bolus"),
          count: bolusInsulin,
          pct: ((bolusInsulin / totalDailyInsulin) * 100).toFixed(0),
        },
      ];
    ...
    function appendProfileSwitch(context, treatment) {
      if (!treatment.cutting && !treatment.profile) {
        return;
      }
