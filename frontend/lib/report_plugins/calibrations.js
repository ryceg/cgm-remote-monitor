...
calibrations.report = function report_calibrations(
  ...
  sorteddaystoshow.forEach(function (day) {
    treatments = treatments.concat(
      datastorage[day].treatments.filter(function (t) {
  ...
  mbgs.forEach(function (mbg) {
    calcmbg(mbg);
  });

  var events = treatments
    .concat(cals)
    .concat(mbgs)
    .sort(function (a, b) {
  ...
  for (var i = 0; i < events.length; i++) {
    var e = events[i];
    colorindex =
      e.device !== undefined ? (colorindex + 1) % colors.length : colorindex;
  ...
  function drawChart() {
    ...
    xScale2 = d3.scaleLinear().domain([0, maxBG]);

    yScale2 = d3.scaleLinear().domain([0, 400000]);
    ...
    charts.attr("width", width).attr("height", height);
    ...
    calibration_context.append("g").attr("class", "x axis");

    // create the y axis container
    calibration_context.append("g").attr("class", "y axis");
    ...
    [50000, 100000, 150000, 200000, 250000, 300000, 350000, 400000].forEach(
      function (li) {
        calibration_context
          .append("line")
