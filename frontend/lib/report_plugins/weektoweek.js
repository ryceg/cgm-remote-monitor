...
function init (ctx) {
  ...
  weektoweek.report = function report_weektoweek (datastorage, sorteddaystoshow, options) {
    ...
    var startDay = dayjs(sorteddaystoshow[0] + ' 00:00:00');

    sorteddaystoshow.forEach(function eachDay (day) {
      var weekNum = Math.abs(dayjs(day + ' 00:00:00').diff(startDay, 'weeks'));
    ...
    weekstoshow.forEach(function eachWeek (week) {
      var sgvData = [];
      var weekStart = dayjs(week[0] + ' 00:00:00');

      week.forEach(function eachDay (day) {
        var dayNum = Math.abs(dayjs(day + ' 00:00:00').diff(weekStart, 'days'));

        datastorage[day].sgv.forEach(function eachSgv (sgv) {
          var sgvWeekday = dayjs(sgv.date).day();
          ...
          sgvData.push({
            'color': sgvColor
            , 'date': dayjs(sgv.date).subtract(dayNum, 'days').toDate()
