...
hourlystats.report = function report_hourlystats(
  datastorage,
  sorteddaystoshow,
  options,
) {
  ...
  [
    ...
    var tr = $("<tr>");
    var display = new Date(0, 0, 1, hour, 0, 0, 0)
      .toLocaleTimeString()
    ...
    var dev = ss.standard_deviation(
      pivotedByHour[hour].map(function (r) {
        return r.sgv;
      }),
    );
    stats.push([
  ...
  sorteddaystoshow.forEach(function (day) {
    if (datastorage[day].netBasalPositive) {
      ...
      for (var h = 0; h < 24; h++) {
        ...
        totalNet[h] += net;
        var color =
          Math.abs(net) < 0.019 ? "black" : net < 0 ? "red" : "lightgreen";
        $(
          "<td>" +
    ...
    for (var h = 0; h < 24; h++) {
      ...
      $(
        '<td style="background-color:' +
          ...
          '";>' +
          '<span style="color:black;">' +
          (totalNegative[h] / days).toFixed(2) +
          " (" +
          negativesCount[h] +
          ")" +
          "</span>" +
          "<br>" +
          '<span style="color:black;">' +
          (totalPositive[h] / days).toFixed(2) +
          " (" +
          positivesCount[h] +
          ")" +
          "</span>" +
  ...
  report.append(
    translate("(temp basals must be rendered to display this report)"),
  );
  report.append("<br><br>");
  report.append(table);
};
