      ...
      $("<tr>")
        ...
        .append(
          $("<td>")
            .attr("width", "10%")
            .attr("align", "center")
            .append(valueOrEmpty(tr.insulin)),
        )
        .append(
          $("<td>")
            .attr("width", "10%")
            .attr("align", "center")
            .append(valueOrEmpty(tr.carbs)),
        )
        .append(
          $("<td>").attr("width", "10%").append(valueOrEmpty(tr.enteredBy)),
        )
  ...
  $.ajax("/api/v1/treatments.json?&find[created_at][$gte]=" + nowiso, {
    ...
    success: function (records) {
      ...
      $status
        .hide()
        .text(
          translate("Database contains %1 future records", {
      ...
      futureitems.actions[0].confirmText = translate(
  ...
  $.ajax("/api/v1/entries.json?&find[date][$gte]=" + now + "&count=288", {
    ...
    success: function (records) {
      ...
      $status
        ...
        .fadeIn("slow");
      futureitems.actions[1].confirmText = translate(
        "Remove %1 selected records?",
        { params: [records.length] },
      );
    },
