...
describe("loop", function () {
  ...
  it("should show errors", function (done) {
    ...
    var sbx = sandbox.clientInit(ctx, errorTime.valueOf(), {
      devicestatus: statuses,
    });
  ...
  it("should generate an alert for a stuck loop", function (done) {
    ...
    var sbx = sandbox.clientInit(ctx, now.clone().add(2, "hours").valueOf(), {
      devicestatus: statuses,
    });
  ...
  it("should handle virtAsst requests", function (done) {
    ...
    loop.virtAsst.intentHandlers[0].intentHandler(
      function next(title, response) {
        ...
        loop.virtAsst.intentHandlers[1].intentHandler(
          function next(title, response) {
            title.should.equal("Last Loop");
            response.should.equal(
              "The last successful loop was a few seconds ago",
            );
            done();
          },
          [],
