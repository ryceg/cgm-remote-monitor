...
describe("COB", function () {
  ...
  it("should calculate IOB, multiple treatments", function () {
    ...
    var after100 = cob.cobTotal(
      treatments,
      devicestatus,
      profile,
      new Date("2015-05-29T02:03:49.827Z").getTime(),
    );
    var before10 = cob.cobTotal(
      treatments,
      devicestatus,
      profile,
      new Date("2015-05-29T03:45:10.670Z").getTime(),
    );
    var after10 = cob.cobTotal(
      treatments,
      devicestatus,
      profile,
      new Date("2015-05-29T03:45:11.670Z").getTime(),
    );
  ...
  it("should calculate IOB, single treatment", function () {
    ...
    var result1 = cob.cobTotal(
      treatments,
      devicestatus,
      profile,
      rightAfterCorrection,
    );
  ...
  it("should handle virtAsst requests", function (done) {
    ...
    cob.virtAsst.intentHandlers[0].intentHandler(
  ...
  describe("from devicestatus", function () {
    ...
    var treatmentCOB = cob.fromTreatments(
      treatments,
      OPENAPS_DEVICESTATUS,
      profile,
      time,
    ).cob;
    ...
    it("should fall back to treatments if openaps devicestatus is present but empty", function () {
      ...
      cob
        .cobTotal(treatments, devicestatus, profile, time)
        .cob.should.equal(treatmentCOB);
    });

    it("should fall back to treatments if openaps devicestatus is present but too stale", function () {
      var devicestatus = [
        _.merge(OPENAPS_DEVICESTATUS, {
          mills: time - cob.RECENCY_THRESHOLD - 1,
          openaps: {
            enacted: { COB: 5, timestamp: time - cob.RECENCY_THRESHOLD - 1 },
          },
        }),
      ];
    ...
    it("should return COB data from OpenAPS", function () {
      var devicestatus = [
        _.merge(OPENAPS_DEVICESTATUS, {
          mills: time - 1,
          openaps: { enacted: { COB: 5, timestamp: time - 1 } },
        }),
      ];
      cob.cobTotal(treatments, devicestatus, profile, time).should.containEql({
        cob: 5,
