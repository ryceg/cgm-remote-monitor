...
describe("IOB", function () {
  ...
  it("should handle virtAsst requests", function (done) {
    ...
    iob.virtAsst.intentHandlers[0].intentHandler(
      function next(title, response) {
        title.should.equal("Current IOB");
        response.should.equal("You have 1.50 units of insulin on board");

        iob.virtAsst.rollupHandlers[0].rollupHandler(
          [],
          sbx,
          function callback(err, response) {
            should.not.exist(err);
            response.results.should.equal(
              "and you have 1.50 units of insulin on board.",
            );
  ...
  describe("from treatments", function () {
    it("should calculate IOB", function () {
      ...
      var afterSomeTime = iob.calcTotal(
        treatments,
        [],
        profile,
        time + 60 * 60 * 1000,
    ...
    it("should not show a negative IOB when approaching 0", function () {
      ...
      var whenApproaching0 = iob.calcTotal(
        treatments,
        [],
        undefined,
  ...
  describe("from devicestatus", function () {
    ...
    it("should fall back to treatments if openaps devicestatus is present but empty", function () {
      ...
      iob
        .calcTotal(treatments, devicestatus, profile, time)
        .iob.should.equal(treatmentIOB);
    });

    it("should fall back to treatments if openaps devicestatus is present but too stale", function () {
      var devicestatus = [
        _.merge(OPENAPS_DEVICESTATUS, {
          mills: time - iob.RECENCY_THRESHOLD - 1,
          openaps: { iob: { timestamp: time - iob.RECENCY_THRESHOLD - 1 } },
        }),
      ];
    ...
    it("should return IOB data from Loop", function () {
      ...
      var devicestatus = [
        _.merge(LOOP_DEVICESTATUS, {
          mills: time - 1,
          loop: { iob: { timestamp: time - 1 } },
        }),
      ];
      iob.calcTotal(treatments, devicestatus, profile, time).should.containEql({
        iob: 0.75,
    ...
    it("should return IOB data from openaps from multiple devices", function () {
      var devicestatus = [
        _.merge(OPENAPS_DEVICESTATUS, {
          mills: time - 1000,
          openaps: { iob: { timestamp: time - 1000 } },
        }),
        _.merge(OPENAPS_DEVICESTATUS, {
          mills: time - 1,
          openaps: { iob: { timestamp: time - 1 } },
        ...
        _.merge(OPENAPS_DEVICESTATUS, {
          mills: time - 20000,
          openaps: { iob: { timestamp: time - 20000 } },
        }),
      ];
      iob.calcTotal(treatments, devicestatus, profile, time).should.containEql({
        iob: 0.047,
