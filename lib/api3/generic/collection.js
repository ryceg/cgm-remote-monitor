...
const apiConst = require('../const.json')
  ...
  , deleteOperation = require('./delete/operation')  , historyOperation = require('./history/operation')
  , bulkCreateOperation = require('./bulk/operation').bulkCreateOperation
  ;
...
function Collection ({ ctx, env, app, colName, storageColName, fallbackGetDate, dedupFallbackFields,
  ...
  self.mapRoutes = function mapRoutes () {
    const prefix = '/' + colName
      ...
      , prefixBulk = prefix + '/bulk'
      ;


    // GET /{collection}
    app.get(prefix, searchOperation(ctx, env, app, self));    // POST /{collection}
    app.post(prefix, createOperation(ctx, env, app, self));

    // POST /{collection}/bulk
    app.post(prefixBulk, bulkCreateOperation(ctx, env, app, self));
