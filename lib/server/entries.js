...
function storage (env, ctx) {
  ...
  function persist (fn) { // fn is the callback from the caller, e.g. insert_entries's done.
    ...
    const dataCollectorStream = new Writable({
      ...
      final(callback) {
        // When dataCollectorStream finishes, 'create' is called.
        // 'create' will then call 'fn' with the outcome.
        create(collectedData, fn);
        callback(); // Signal Writable's finalization is done
      }
    });
