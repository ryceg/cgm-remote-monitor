...
function configure (app, wares, ctx, env) {
  ...
  function force_typed_data (opts) {
    return new Transform({
      ...
      transform(data, encoding, callback) {
        ...
        // if element has no date, make one now
        if (!data.date) {
          data.date = (new Date( )).getTime( );
        }
        // if element has no dateString, make one now
        if (!data.dateString) {
          data.dateString = (new Date(data.date)).toISOString( );
        }
        // TODO: verify data type is valid, and known
        // TODO: verify data has all required fields for its type
        // TODO: verify fields have correct data types
        // TODO: verify no extra fields are present
        this.push(data);
        callback();
      }
    });
  ...
  function format_entries (req, res) {
    ...
    const output = Readable.from(res.entries || []);
    ...
    const createWritable = (formatter) => {
      const chunks = [];
      return new Writable({
        objectMode: true,
    ...
    return res.format({
      'text/plain': function() {
        pipeline(
          ...
          createWritable(function(err, out) {
            var outputText = formatWithSeparator(out, "\t");
            res.send(outputText);
          }),
          (err) => { if (err) console.error('Pipeline failed for text/plain', err); }
        );
      }
      , 'text/tab-separated-values': function() {
        pipeline(
          ...
          (err) => { if (err) console.error('Pipeline failed for text/tab-separated-values', err); }
        );
      }
      , 'text/csv': function() {
        pipeline(
          ...
          (err) => { if (err) console.error('Pipeline failed for text/csv', err); }
        );
      }
      , 'application/json': function() {
        pipeline(
          ...
          (err) => { if (err) console.error('Pipeline failed for application/json', err); }
        );
      }
      , 'default': function() {
        pipeline(
          ...
          (err) => { if (err) console.error('Pipeline failed for default', err); }
        );
      }
    });
  }
   ...
   * @description Middleware to process and persist or preview uploaded entries.
   *
   * This function handles incoming HTTP requests containing entry data.
   * It extracts entry data from the request body, which can be a single JSON object or an array of objects.
   * Each entry is then purified using `ctx.purifier`.
   *
   * If `req.persist_entries` is true (typically for POST /entries), the incoming data is piped
   * through `entries.persist()`, which is a writable stream responsible for saving the data.
   * The `done` callback is invoked by the `entries.persist()` stream's logic upon completion or error.
   *
   * If `req.persist_entries` is false (typically for POST /entries/preview), the data is piped
   * through `entries.map()` (a transform stream) and then into a `Writable` stream (`previewCollectorStream`)
   * that collects all data chunks. Once all data is collected, the `done` callback is invoked with the
   * aggregated data.
   *
   * The `done` callback, regardless of the path taken, stores the results (or error) on `res.entries`
   * and `res.entries_err` respectively, and then calls `next()` to pass control to the next middleware.
   *
   * Error handling is implemented within the `pipeline` callbacks to ensure that `done` is called
   * appropriately in case of stream errors.
   *
   * @param {object} req - The Express request object. Expected to have `req.body` containing entry data
   *                       and `req.persist_entries` (boolean) to determine if data should be saved.
   * @param {object} res - The Express response object. Results are attached as `res.entries`.
  ...
  function insert_entries (req, res, next) {
    ...
    if (req.body && typeof req.body === 'object' && 'date' in req.body && !Array.isArray(req.body)) {
    ...
    // req.persist_entries is expected to be true for actual persistence (e.g. POST /entries)
    // and false or undefined for preview (e.g. POST /entries/preview)
    if (req.persist_entries) {
      pipeline(
        Readable.from(incoming),
        entries.persist(done), // entries.persist is from lib/server/entries.js; it takes 'done' and calls it. Returns Writable.
        (err) => {
          // This callback handles errors from Readable.from(incoming) or if the initial piping
          // to entries.persist() fails before entries.persist() itself handles the error.
          if (err) {
            return done(err);
          }
          // If no error here, 'done' is expected to be called by the entries.persist() stream's internal logic.
        }
      );
    } else { // Preview mode
      const dataCollector = [];
      const previewCollectorStream = new Writable({
        objectMode: true,
        write(chunk, encoding, callback) {
          dataCollector.push(chunk);
          callback();
        },
        final(callback) {
          done(null, dataCollector); // Call 'done' with the collected data
          callback();
        },
        destroy(err, callback) {
          // Ensure 'done' is called with an error if the stream is destroyed prematurely.
          if (err && !this.writableFinished && !this.writableEnded) {
            done(err);
          }
          callback(err);
        }
      ...
      pipeline(
        Readable.from(incoming),
        entries.map(), // .map() is a Transform stream from the lib/server/entries.js storage instance
        previewCollectorStream,
        (err) => {
          // This callback handles errors from Readable.from, entries.map, or the piping itself.
          if (err) {
            return done(err);
          }
          // If no error here, 'done' is expected to be called by previewCollectorStream.final().
        }
      );
    }
  }
