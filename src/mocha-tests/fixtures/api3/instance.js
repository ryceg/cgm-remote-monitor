...
function configure() {
  ...
  self.addSecuredOperations = function addSecuredOperations(instance) {
    instance.get = (url, jwt) =>
      addJwt(request(instance.baseUrl).get(url), jwt);

    instance.post = (url, jwt) =>
      addJwt(request(instance.baseUrl).post(url), jwt);

    instance.put = (url, jwt) =>
      addJwt(request(instance.baseUrl).put(url), jwt);

    instance.patch = (url, jwt) =>
      addJwt(request(instance.baseUrl).patch(url), jwt);

    instance.delete = (url, jwt) =>
      addJwt(request(instance.baseUrl).delete(url), jwt);
  };
  ...
  self.create = function createHttpServer({
    ...
    return new Promise(function (resolve, reject) {
      try {
        ...
        require("../../../lib/server/bootevent")(instance.env, language).boot(
          function booted(ctx) {
            instance.ctx = ctx;
            ...
            instance.app.use(
              ...
              instance.ctx.authorization.endpoints,
            );

            const transport = useHttps ? https : http;

            instance.server = transport
              .createServer(instance.env.ssl || {}, instance.app)
              .listen(0);
            ...
            self
              .bindSocket(storageSocket, instance)
              .then((socket) => {
                instance.clientSocket = socket;

                console.log(
                  `Started ${useHttps ? "SSL" : "HTTP"} instance on ${instance.baseUrl}`,
                );
                hasBooted = true;
                resolve(instance);
              })
