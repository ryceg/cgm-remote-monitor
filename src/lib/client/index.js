...
const $ =
  (global && /** @type {{ $?: JQueryStatic }} */ (global).$) ||
  require("jquery");
...
const Alarm = require("../notifications").Alarm;

const FORMAT_TIME_12 = "%-I:%M %p";
const FORMAT_TIME_12_COMPACT = "%-I:%M";
const FORMAT_TIME_24 = "%H:%M%";
const FORMAT_TIME_12_SCALE = "%-I %p";
const FORMAT_TIME_24_SCALE = "%H";
...
class Client {
  constructor() {
    this.hashauth = new HashAuth(this, $);
    ...
    /** @type {ReturnType<import("../sandbox")> | undefined} */
    this.nowSBX = undefined;
    /** @type {import("../types").Sgv | undefined} */
    this.latestSGV = undefined;

    /**
     * @type {{
     *       data?: ReturnType<import("../data/ddata")>;
     *       loadedMills: number;
     *       loadStartedMills: number;
     *     }
     *   | undefined}
     */
    ...
    /** @type {BrowserUtils} */
    this.browserUtils;
    /** @type {BrowserSettings} @protected */
    this.browserSettings;
    /** @type {BrowserSettings["settings"]} */
    this.settings;
    /** @type {ReturnType<import("../language")>["translate"]} */
    this.translate;
    /** @type {ReturnType<import("../language")>} */
    this.language;
    /** @type {Record<"current" | "DAY" | "NIGHT", number>} */
    this.opacity;
    /** @type {string} */
    this.alarmSound;
    /** @type {string} */
    this.urgentAlarmSound;
    /** @type {number} */
    this.now;
    /** @type {number | Date} */
    this.dataLastUpdated;
    /** @type {Date} */
    this.lastPluginUpdateTime;
    /** @type {ReturnType<import("../data/ddata")>} */
    this.ddata;
    /** @type {number} */
    this.defaultForecastTime;
    /** @type {number} */
    this.forecastTime;
    ...
    this.direction;
    /** @type {ReturnType<import("../plugins/errorcodes")>} */
    this.errorcodes;
    /** @type {import("../sandbox").ClientInitializedSandbox} */
    this.sbx;
    /** @type {ReturnType<import("./renderer")>} */
    this.renderer;
    /** @type {AdminNotifiesClient} */
    this.notifies;
    /** @type {number} */
    this.focusRangeMS;
    /** @type {ReturnType<import("../profilefunctions")>} */
    this.profilefunctions;
    /** @type {ReturnType<import("./careportal")>} */
    this.careportal;
    /** @type {ReturnType<import("./boluscalc")>} */
    this.boluscalc;
     ...
     *   notifications: ReturnType<import("../notifications")>;
     * }} ClientCtx
     */
    /** @type {ClientCtx} */
    this.ctx;
  ...
  headers() {
    ...
    } else if (this.hashauth) {
      return {
        "api-secret": this.hashauth.hash(),
      };
    }
    return {};
  }
  ...
  init(callback) {
    ...
    const token = this.browserUtils.queryParams().token;
    const secret =
      this.hashauth.apisecrethash || Storages.localStorage.get("apisecrethash");

    let src = "/api/v1/status.json?t=" + new Date().getTime();
    if (secret) src += "&secret=" + secret;
    else if (token) src += "&token=" + token;
    ...
    $.ajax({
      ...
      .done((serverSettings) => {
        if (serverSettings.runtimeState !== "loaded") {
          ...
          window.setTimeout(() => this.init(callback), 5000);
          return;
        }
        this.settingsFailed = false;
        this.loadLanguage(serverSettings, callback);
      })
      .fail((jqXHR) => {
        ...
        let lang =
          Storages.localStorage.get("language")?.toString() ||
        ...
        this.translate = language.translate;
        // auth failed, hide loader and request for key
        $("#centerMessagePanel").hide();
        this.hashauth.requestAuthentication(() => {
          window.setTimeout(() => this.init(callback), 5000);
        });
      });
  }
  ...
  /**
   * @param {StatusJSONResponse} serverSettings
   * @param {() => void} [callback]
   */
  loadLanguage(serverSettings, callback) {
    $("#loadingMessageText").html("Loading language file");

    this.browserSettings = new BrowserSettings(this, serverSettings, $);
    /** @type {ReturnType<import("../settings")>} */
    this.settings = this.browserSettings.settings;
    console.log("language is", this.settings.language);

    $.ajax({
      method: "GET",
      url: "/translations/" + language.getFilename(this.settings.language),
    })
      .done((localization) => {
        language.offerTranslations(localization);
        console.log("Application appears to be online");
        $("#centerMessagePanel").hide();
        this.load(serverSettings, callback);
      })
      .fail(() => {
  ...
  afterAuth(isAuthenticated) {
    //TODO better typeguards
    if (!this.settings || !this.notifies || !this.editButton) return;

    const treatmentCreateAllowed = this.authorized
      ? this.authorized.check("api:treatments:create")
      : isAuthenticated;
    const treatmentUpdateAllowed = this.authorized
      ? this.authorized.check("api:treatments:update")
      : isAuthenticated;

    $("#lockedToggle")
      .on("click", (event) => this.hashauth.requestAuthentication(event))
      .toggle(
        !treatmentCreateAllowed &&
          this.settings.showPlugins.includes("careportal")
      );
    $("#treatmentDrawerToggle").toggle(
      treatmentCreateAllowed && this.settings.showPlugins.includes("careportal")
    );
    $("#boluscalcDrawerToggle").toggle(
      treatmentCreateAllowed && this.settings.showPlugins.includes("boluscalc")
    );

    if (isAuthenticated) this.notifies.updateAdminNotifies();

    // Edit mode
    this.editButton.toggle(this.settings.editMode && treatmentUpdateAllowed);
    this.editButton.on("click", (event) => {
      if (!this.editButton || !this.renderer || !this.chart) return;
      this.editMode = !this.editMode;
      if (this.editMode) {
        this.renderer.drawTreatments(this);
        this.editButton.find("i").addClass("selected");
      } else {
        this.chart.focus
          .selectAll(".draggable-treatment")
          .style("cursor", "default")
          .on("mousedown.drag", null);
        this.editButton.find("i").removeClass("selected");
      }
   ...
   * @param {() => void} [callback]
   */
  // * @this {Stage1InitializedClient}
  load(serverSettings, callback) {
    /** @type {ReturnType<import("./chart")> | undefined} */
    this.chart = undefined;
    this.socket;
    this.alarmSocket;
    this.isInitialData = false;
    /** @type {Record<"current" | "DAY" | "NIGHT", number>} */
    this.opacity = { current: 1, DAY: 1, NIGHT: 0.5 };
    /** @type {Record<string, import("../notifications").Alarm>} */
    this.clientAlarms = {};
    this.alarmInProgress = false;
    this.alarmMessage;
    this.currentNotify;
    /** @type {{ received: number; title: string; message: string } | null} */
    this.currentAnnouncement;
    this.alarmSound = "alarm.mp3";
    this.urgentAlarmSound = "alarm2.mp3";
    this.previousNotifyTimestamp;

    this.now = Date.now();
    this.dataLastUpdated = new Date(0);
    this.lastPluginUpdateTime = new Date(0);
    this.ddata = require("../data/ddata")();
    this.defaultForecastTime = times.mins(30).msecs;
    /** @type {number} */
    this.forecastTime = this.now + this.defaultForecastTime;
    /** @type {import("../types").Entry[]} */
    this.entries = [];
    this.ticks = require("./ticks");

    //containers
    this.container = $(".container");
    this.bgStatus = $(".bgStatus");
    this.currentBG = $(".bgStatus .currentBG");
    this.majorPills = $(".bgStatus .majorPills");
    this.minorPills = $(".bgStatus .minorPills");
    this.statusPills = $(".status .statusPills");
    ...
    this.settings = new BrowserSettings(this, serverSettings, $).settings;

    language.set(this.settings.language)?.DOMtranslate($);
    this.translate = language.translate;
    ...
    this.ctx = {
      ...
      language,
      levels,
      notifications: require("../notifications")(this.settings, {
        bus,
        levels,
      }),
    };

    this.sbx = sandbox.clientInit(this.ctx, this.now);
    this.renderer = require("./renderer")(this, d3);

    //After plugins are initialized with browser settings;
    this.browserSettings?.loadAndWireForm();

    this.notifies = new AdminNotifiesClient(this, $);

    if (serverSettings && serverSettings.authorized) {
      this.authorized = {
        exp: 0,
        iat: 0,
        ...serverSettings.authorized,
        lat: Date.now(),
        shiros:
          serverSettings.authorized.permissionGroups?.map((group) => {
            const shiro = shiroTrie.newTrie();
            shiro.add(...group);
            return shiro;
          }) ?? [],
        check(permission) {
          return !!this.shiros?.some((s) => s.check(permission));
        },
      };
    }

    this.hashauth.initAuthentication((success) => this.afterAuth(success));

    this.focusRangeMS = times.hours(this.settings.focusHours).msecs;
    $(`.focus-range li[data-hours="${this.settings.focusHours}"]`).addClass(
      "selected"
    );

    const profile = require("../profilefunctions")(null, this.ctx);
    this.profilefunctions = profile;

    this.careportal = require("./careportal")(this, $);
    this.boluscalc = require("./boluscalc")(this, $);

    this.editMode = false;

    document.addEventListener("visibilitychange", () => {
      const prevHidden = this.documentHidden;
      this.documentHidden = document.hidden;

      if (prevHidden && !this.documentHidden) {
        console.info("Document now visible, updating - " + new Date());
        this.refreshChart(true);
      }
    });

    window.onresize = (event) => this.refreshChart(!!event);

    this.updateClock();
    this.updateTimeAgoSoon();

    const silenceDropdown = new Dropdown("#silenceBtn");
    const viewDropdown = new Dropdown("#viewMenu");

    $(".bgButton").on("click", (e) => {
      if (this.alarmingNow()) silenceDropdown.open(e);
    });

    $(".focus-range li").on("click", (e) => {
      const li = $(e.target);
      ...
      const hours = Number(li.data("hours"));
      this.focusRangeMS = times.hours(hours).msecs;
      Storages.localStorage.set("focusHours", hours.toString());
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    this.socket = io.connect({ transports: ["polling"] });
    this.alarmSocket = io.connect("/alarm", {
      multiplex: true,
      transports: ["polling"],
    });
    ...
    this.socket.on("retroUpdate", (retroData) => {
      console.info(
        "got retroUpdate",
        retroData,
        new Date(this.now ?? Date.now())
      );
      this.retro = {
    ...
    this.socket.on("connect", () => {
      ...
      this.authorizeSocket(callback);
    });

    this.alarmSocket.on("connect", () => this.subscribeForAlarms());

    this.alarmSocket.on(
      "notification",
      /** @param {import("../types").Notify} notify */ (notify) => {
        console.log("notification from server:", notify);
        if (
          notify.timestamp &&
          this.previousNotifyTimestamp !== notify.timestamp
        ) {
          /** @type {number} */
          this.previousNotifyTimestamp = notify.timestamp;
          this.plugins.visualizeAlarm(
            this.sbx,
            notify,
    ...
    this.alarmSocket.on(
      ...
      /** @param {import("../types").Notify} notify */ (notify) => {
        ...
        this.currentAnnouncement = { ...notify, received: Date.now() };
        this.updateTitle();
      }
    );
    ...
    this.alarmSocket.on(
      ...
      /** @param {import("../types").Notify} notify */ (notify) => {
        // TODO types
        if (!this.alarmSound || !this.chart) return;

        console.info("alarm received from server");

        if (this.isAlarm()) {
          console.log("Alarm raised!");
          this.generateAlarm(this.alarmSound, notify);
        } else {
          console.info(
    ...
    this.alarmSocket.on(
      ...
      /** @param {import("../types").Notify} notify */ (notify) => {
        // TODO types
        if (!this.urgentAlarmSound || !this.chart) return;

        console.info("urgent alarm received from server");

        if (this.isAlarm()) {
          console.log("Urgent alarm raised!");
          this.generateAlarm(this.urgentAlarmSound, notify);
        } else {
          console.info(
    ...
    this.alarmSocket.on(
      ...
      /** @param {import("../types").Notify} notify */ (notify) => {
        ...
        this.stopAlarm(false, undefined, notify);
      }
    );
    ...
    this.container.toggleClass(
      "has-minor-pills",
      this.plugins.hasShownType("pill-minor", this.settings)
    );
  }
  ...
  dataExtent() {
    if (!this.entries || !this.entries.length) {
      this.now ??= Date.now();
      return [
        new Date(this.now - times.hours(this.history).msecs),
        new Date(this.now),
      ];
    }

    return [
      this.entryToDate(this.entries[0]),
      this.entryToDate(this.entries[this.entries.length - 1]),
    ];
  }

  bottomOfPills() {
    // the offsets might not exist for some tests
    const bottomOfPirmary =
      (this.primary?.offset()?.top ?? 0) + (this.primary?.height() ?? 0);
    const bottomOfMinorPills =
      (this.minorPills?.offset()?.top ?? 0) + (this.minorPills?.height() ?? 0);
    const bottomOfStatusPills =
      (this.statusPills?.offset()?.top ?? 0) +
      (this.statusPills?.height() ?? 0);

    return Math.max(bottomOfPirmary, bottomOfMinorPills, bottomOfStatusPills);
  }
  ...
  formatTime(time, compact) {
    ...
    const formatted = d3.timeFormat(timeFormat)(time);

    if (this.settings?.timeFormat !== 24) return formatted.toLowerCase();
    return formatted;
  }
  ...
  getTimeFormat(isForScale, compact) {
    if (this.settings?.timeFormat === 24) {
      return isForScale ? FORMAT_TIME_24_SCALE : FORMAT_TIME_24;
    }
   ...
   */
  scaleBg(bg) {
    return this.settings?.units === "mmol" ? units.mgdlToMMOL(bg) : bg;
  }

  /** @protected */
  generateTitle() {
    if (!this.timeago || !this.sbx) return "";

    const status = this.timeago.checkStatus(this.sbx);

    if (status !== "current") {
      const lastSGVEntry = this.sbx.lastSGVEntry();
      if (!lastSGVEntry) return "";

      const ago = this.timeago.calcDisplay(lastSGVEntry, this.sbx.time);
      return `${ago.value} ${ago.label} - `;
    }

    if (!this.latestSGV) return "";

    const currentMgdl = this.latestSGV.mgdl;

    if (currentMgdl < 39) {
      return `${this.errorcodes.toDisplay(currentMgdl)} - `;
    }

    const delta = this.nowSBX?.properties.delta;
    if (!delta) return "";

    return `${this.scaleBg(currentMgdl)} ${delta.display} ${this.direction.info(this.latestSGV).label}`;
  }

  /** @protected */
  resetCustomTitle() {
    $(".customTitle").text(this.settings?.customTitle ?? "Nightscout");
  }
  ...
  checkAnnouncement() {
    if (
      !this.currentAnnouncement ||
      Date.now() - this.currentAnnouncement.received >= times.mins(5).msecs
    ) {
    ...
    const message =
      this.currentAnnouncement.message.length > 1
  ...
  /** @protected */
  windowTitle() {
    const announcementStatus = this.checkAnnouncement();
    if (this.alarmMessage && this.alarmInProgress) {
      $(".customTitle").text(this.alarmMessage);
      return this.alarmMessage + ": " + this.generateTitle();
    }
    if (announcementStatus.inProgress && announcementStatus.message) {
      return announcementStatus.message + ": " + this.generateTitle();
    }
  ...
  updateTitle() {
    const announcementStatus = this.checkAnnouncement();
    this.container?.toggleClass("announcing", announcementStatus.inProgress);
   ...
   *
   * @param {boolean} [skipBrushing]
   * @protected
   */
  updateBrushToNow(skipBrushing) {
    const brushExtent = this.dataExtent();
    brushExtent[0] = new Date(
      brushExtent[1].getTime() - (this.focusRangeMS ?? 0)
    );

    if (this.chart?.theBrush) {
      this.chart.theBrush.call(this.chart.brush);
      this.chart.theBrush.call(
        this.chart.brush.move,
        brushExtent.map(this.chart.xScale2)
      );
    }

    if (!skipBrushing) this.brushed();
  }

  /** @protected */
  alarmingNow() {
    return this.container?.hasClass("alarming");
  }

  /** @protected */
  inRetroMode() {
    return this.chart?.inRetroMode();
  }

  brushed() {
    if (!this.chart?.theBrush || brushing) return;
    brushing = true;

    // default to most recent focus period
    let brushExtent = this.dataExtent();
    brushExtent[0] = new Date(
      brushExtent[1].getTime() - (this.focusRangeMS ?? 0)
    );

    const brushNode = this.chart.theBrush.node();
    if (!brushNode) return;
    const brushedRange = d3.brushSelection(brushNode);

    if (brushedRange) {
      brushExtent = /** @type {[number, number]} */ (brushedRange).map(
        this.chart.xScale2.invert
      );
    }
    ...
    if (
      !brushedRange ||
      brushExtent[1].getTime() - brushExtent[0].getTime() !== this.focusRangeMS
    ) {
      if (
        brushExtent[0].getTime() + (this.focusRangeMS ?? 0) >
        this.dataExtent()[1].getTime()
      ) {
        brushExtent[0] = new Date(
          brushExtent[1].getTime() - (this.focusRangeMS ?? 0)
        );
    ...
    const nowDate = this.inRetroMode()
      ? brushExtent[1]
      : new Date(this.now ?? 0);

    if (this.inRetroMode()) {
      $("#currentTime")
        .text(this.formatTime(nowDate, true))
    ...
    const nowData =
      this.entries?.filter(
        (d) => d.type === "sgv" && d.mills <= brushExtent[1].getTime()
      ) ?? [];
    const focusPoint = nowData.at(-1);

    if (focusPoint) {
      if (brushExtent[1].getTime() - focusPoint.mills > times.mins(15).msecs) {
        this.clearCurrentSGV();
      } else {
        this.updateCurrentSGV(focusPoint);
      }
      this.updatePlugins(nowDate);
    } else {
      this.clearCurrentSGV();
      this.updatePlugins(nowDate);
    }
    ...
    this.updateTimeAgo();
    if (this.chart.prevChartHeight) {
      this.chart.scroll(nowDate.getTime());
    }

    const top = this.bottomOfPills() + 5;
    $("#chartContainer").css({
      top: top + "px",
      height: ($(window).height() ?? 0) - top - 10,
    });
  ...
  adjustCurrentSGVClasses(value, isCurrent) {
    // TODO types
    if (!this.bgStatus || !this.container || !this.currentBG) return;
    const reallyCurrent = isCurrent && !this.inRetroMode();

    this.bgStatus.toggleClass("current", this.alarmingNow() || reallyCurrent);

    if (!this.alarmingNow()) {
      this.container.removeClass("urgent warning inrange");
      if (reallyCurrent) {
        this.container.addClass(this.sgvToColoredRange(value));
      }
    }

    this.currentBG.toggleClass("icon-hourglass", value === 9);
    this.currentBG.toggleClass("error-code", value < 39);
    this.currentBG.toggleClass("bg-limit", value === 39 || value > 400);
  }
   ...
   * @protected
   */
  updateCurrentSGV(entry) {
    // TODO types
    if (!this.timeago || !this.sbx || !this.currentBG || !this.errorcodes)
      return;

    if (entry.mgdl === 9) {
      this.currentBG.text("");
    } else if (entry.mgdl < 39) {
      this.currentBG.html(this.errorcodes.toDisplay(entry.mgdl));
    } else if (entry.mgdl < 40) {
      this.currentBG.text("LOW");
    } else if (entry.mgdl > 400) {
      this.currentBG.text("HIGH");
    } else {
      this.currentBG.text(this.scaleBg(entry.mgdl));
    }

    this.adjustCurrentSGVClasses(
      entry.mgdl,
      this.timeago.checkStatus(this.sbx) === "current"
    );
  }

  /**
   * @param {import("../types").DeviceStatus[] | undefined} retro
   * @param {import("../types").DeviceStatus[]} ddata
   * @protected
   */
  mergeDeviceStatus(retro, ddata) {
    if (!retro) return ddata;

    const foundInRetro = retro.map((x) => ({
      ...x,
      ...ddata.find((y) => y._id === x._id),
    }));

    const missingInRetro = ddata.filter((y) =>
      retro.some((x) => x._id !== y._id)
    );

    return [...foundInRetro, ...missingInRetro];
  }
   ...
   */
  updatePlugins(time) {
    if (time > this.lastPluginUpdateTime && time > this.dataLastUpdated) {
      if (time.getTime() - this.lastPluginUpdateTime.getTime() < 1000) {
        return; // Don't update the plugins more than once a second
      }
      this.lastPluginUpdateTime = time;
    }

    //TODO: doing a clone was slow, but ok to let plugins muck with data?
    //var ddata = client.ddata.clone();

    this.ddata.inRetroMode = this.inRetroMode();
    this.ddata.profile = this.profilefunctions;

    // retro data only ever contains device statuses
    // Cleate a clone of the data for the sandbox given to plugins

    const mergedStatuses = this.mergeDeviceStatus(
      this.retro?.data?.devicestatus,
      this.ddata.devicestatus
    );

    const clonedData = this.ddata.clone();
    clonedData.devicestatus = mergedStatuses;

    this.sbx = sandbox.clientInit(
      this.ctx,
    ...
    this.plugins.setProperties(this.sbx);

    //only shown plugins get a chance to update visualisations
    this.plugins.updateVisualisations(this.sbx);

    const viewMenu = $("#viewMenu");
    viewMenu.empty();
    this.sbx.pluginBase?.forecastInfos.forEach((info) => {
      const forecastHtml = $(`
        <li>
          <label>
            <input
              type="checkbox"
              data-forecast-type="${info.type}"
              ${this.settings?.showForecast.includes(info.type) ? "checked" : ""}
            />
      ...
      forecastHtml.find("input").change((event) => {
        if (!this.settings) return;

        const checkbox = $(event.target);
        const type = checkbox.attr("data-forecast-type");
        const checked = checkbox.prop("checked");
        if (checked) {
          this.settings.showForecast += " " + type;
        } else {
          this.settings.showForecast = this.settings.showForecast
            .split(" ")
            .filter((forecast) => forecast !== type)
            .join(" ");
        }

        Storages.localStorage.set("showForecast", this.settings.showForecast);

        this.refreshChart(true);
      });

      viewMenu.append(forecastHtml);
    });

    //send data to boluscalc too
    this.boluscalc.updateVisualisations(this.sbx);
  }

  /** @protected */
  clearCurrentSGV() {
    this.currentBG?.text("---");
    this.container?.removeClass("alarming urgent warning inrange");
  }

  /**
   * @param {number} sgv
   * @returns {"grey" | "red" | "yellow" | "#4cff00"}
   * @protected
   */
  sgvToColor(sgv) {
    if (!this.settings) return "grey";
    if (this.settings.theme === "default") return "grey";

    if (sgv > this.settings.thresholds.bgHigh) return "red";
    if (sgv > this.settings.thresholds.bgTargetTop) return "yellow";
    if (
      sgv >= this.settings.thresholds.bgTargetBottom &&
      sgv <= this.settings.thresholds.bgTargetTop
    )
      return "#4cff00";
    if (sgv < this.settings.thresholds.bgLow) return "red";
    if (sgv < this.settings.thresholds.bgTargetBottom) return "yellow";
  ...
  /**
   * @param {number} sgv
   * @returns {"" | "urgent" | "warning" | "inrange"}
   * @protected
   */
  sgvToColoredRange(sgv) {
    if (!this.settings || this.settings.theme === "default") return "";

    if (sgv > this.settings.thresholds.bgHigh) return "urgent";
    if (sgv > this.settings.thresholds.bgTargetTop) return "warning";
    if (
      sgv >= this.settings.thresholds.bgTargetBottom &&
      sgv <= this.settings.thresholds.bgTargetTop
    )
      return "inrange";
    if (sgv < this.settings.thresholds.bgLow) return "urgent";
    if (sgv < this.settings.thresholds.bgTargetBottom) return "warning";

    return "";
  }

  /** @protected @param {import("../types").Notify} [notify] */
  formatAlarmMessage(notify) {
    if (notify?.isAnnouncement && notify?.message?.length > 1) {
  ...
  setAlarmMessage(notify) {
    this.alarmMessage = this.formatAlarmMessage(notify);
  }

  /** @protected @param {string} file @param {import("../types").Notify} notify */
  generateAlarm(file, notify) {
    this.alarmInProgress = true;

    this.currentNotify = notify;
    this.setAlarmMessage(notify);

    if (!this.alarmingNow()) {
      /** @type {d3.Selection<HTMLAudioElement, unknown, HTMLElement, any>} */
      const els = d3.select(`.audio.alarms audio.${file}`);
      const that = this;
      els.each(function () {
        that.playAlarm(this);
    ...
    this.container
      .addClass("alarming")
      .addClass(file === this.urgentAlarmSound ? "urgent" : "warning");

    const silenceBtn = $("#silenceBtn");
    silenceBtn.empty();

    this.settings.snoozeMinsForAlarmEvent(notify).forEach((mins) => {
      const snoozeOption = $(`
        <li>
          <a data-snooze-time="${times.mins(mins).secs}">
            ${this.translate("Silence for %1 minutes", { params: [mins.toString()] })}
          </a>
        </li>
      `);
      snoozeOption.on("click", (event) => this.snoozeAlarm(event));
  ...
  /** @protected @param {JQuery.ClickEvent} event */
  snoozeAlarm(event) {
    this.stopAlarm(true, $(event.target).data("snooze-time"));
    event.preventDefault();
  }
  ...
  playAlarm(audio) {
    ...
    if (this.browserUtils.queryParams().mute !== "true") {
      audio.play();
    } else {
      this.browserUtils.showNotification("Alarm was muted (?mute=true)");
    }
  }

  /**
   * @param {boolean} isClient
   * @param {number} silenceTime
   * @param {import("../types").Notify} [notify]
   * @protected
   */
  stopAlarm(isClient, silenceTime = times.mins(5).msecs, notify) {
    this.alarmInProgress = false;
    this.alarmMessage = null;
    this.container.removeClass("urgent warning");
    /** @type {d3.Selection<HTMLAudioElement, unknown, HTMLElement, any>} */ (
      d3.selectAll("audio.playing")
    ).each(function () {
      this.pause();
      $(this).removeClass("playing");
    });

    this.browserUtils.closeNotification();
    this.container.removeClass("alarming");

    this.updateTitle();

    const alarm = this.getClientAlarm(
      notify?.level ?? this.currentNotify?.level ?? levels.INFO,
      notify?.group ?? this.currentNotify?.group ?? ""
    );
    ...
    if (alarm) {
      ...
      if (alarm.group === "Time Ago") {
        this.container.removeClass("alarming-timeago");
      }
    } else {
      console.info("No alarm to ack for", notify || this.currentNotify);
    }

    // only emit ack if client invoke by button press
    if (isClient && this.currentNotify) {
      this.alarmSocket.emit(
        "ack",
        this.currentNotify.level,
        this.currentNotify.group,
        silenceTime
      );
    }
  ...
  /** @protected */
  refreshAuthIfNeeded() {
    const clientToken = this.authorized?.token ?? null;
    const token = this.browserUtils.queryParams().token || clientToken;

    if (token && this.authorized) {
      const renewTime =
        this.authorized.exp * 1000 -
        times.mins(15).msecs -
        Math.abs(this.authorized.iat * 1000 - this.authorized.lat);
      const refreshIn = Math.round((renewTime - this.now) / 1000);

      if (this.now > renewTime) {
  ...
  updateClock() {
    ...
    setTimeout(() => this.updateClock(), interval);

    this.updateTimeAgo();
    if (this.chart) this.brushed();

    // Dim the screen by reducing the opacity when at nighttime
    if (this.settings.nightMode) {
      const now = new Date();
      if (
        this.opacity.current !== this.opacity.NIGHT &&
        (now.getHours() > 21 || now.getHours() < 7)
      ) {
        $("body").css({ opacity: this.opacity.NIGHT });
      } else {
        $("body").css({ opacity: this.opacity.DAY });
      }
    }

    this.refreshAuthIfNeeded();
    this.resetRetroIfNeeded();
  }

  /** @protected */
  updateClockDisplay() {
    if (this.inRetroMode()) return;

    this.now = Date.now();
    $("#currentTime")
      .text(this.formatTime(new Date(this.now), true))
      .css("text-decoration", "");
  }

  /** @protected @param {import("../types").Level} level @param {string} group */
  getClientAlarm(level, group) {
    const key = level + "-" + group;

    this.clientAlarms ??= {};
    this.clientAlarms[key] ??= new Alarm(level, group, "");

    return this.clientAlarms[key];
  }

  /** @protected */
  isTimeAgoAlarmType() {
    return this.currentNotify?.group === "Time Ago";
  }

  /** @protected @param {string} status */
  isStale(status) {
    // TODO types
    if (!this.settings) return false;

    return (
      (this.settings.alarmTimeagoWarn && status === "warn") ||
      (this.settings.alarmTimeagoUrgent && status === "urgent")
    );
  }

  /** @protected @param {import("../notifications").Alarm} alarm */
  notAcked(alarm) {
  ...
  checkTimeAgoAlarm(status) {
    ...
    const level = status === "urgent" ? levels.URGENT : levels.WARN;
    const alarm = this.getClientAlarm(level, "Time Ago");

    if (this.isStale(status) && this.notAcked(alarm)) {
      console.info("generating timeAgoAlarm", alarm);
      this.container.addClass("alarming-timeago");

      const lastSGVEntry = this.sbx.lastSGVEntry();
      if (!lastSGVEntry) return;
      const display = this.timeago.calcDisplay(lastSGVEntry, this.sbx.time);

      /** @type {import("../types").Notify} */
      const notify = {
        title:
          this.translate("Last data received") +
          " " +
          display.value +
          " " +
    ...
    this.container.toggleClass("alarming-timeago", status !== "current");

    if (status === "warn") {
      this.container.addClass("warn");
    } else if (status === "urgent") {
      this.container.addClass("urgent");
    }

    if (
      this.alarmingNow() &&
      status === "current" &&
      this.isTimeAgoAlarmType()
    ) {
      this.stopAlarm(true, times.min().msecs);
    }
  }
  ...
  updateTimeAgo() {
    ...
    if (status !== "current") this.updateTitle();

    this.checkTimeAgoAlarm(status);
  }

  /** @protected */
  updateTimeAgoSoon() {
    setTimeout(() => this.updateTimeAgo(), times.secs(10).msecs);
  }
  ...
  refreshChart(updateToNow) {
    ...
    if (updateToNow) this.updateBrushToNow();
  ...
  resetRetroIfNeeded() {
    if (
      this.retro &&
      this.retro.loadedMills > 0 &&
      Date.now() - this.retro.loadedMills > times.mins(5).msecs
    ) {
      this.resetRetro();
  ...
  loadRetroIfNeeded() {
    // TODO types
    if (!this.retro || !this.socket) return;

    const now = Date.now();
    if (now - this.retro.loadStartedMills < times.secs(30).msecs) {
      console.info(
        "retro already loading, started",
        new Date(this.retro.loadStartedMills)
      );
      return;
    }

    if (now - this.retro.loadedMills > times.mins(3).msecs) {
      this.retro.loadStartedMills = now;
      console.info(
        "retro not fresh load started",
        new Date(this.retro.loadStartedMills)
      );
      this.socket.emit("loadRetro", {
        loadedMills: this.retro.loadedMills,
      });
    }
  }

  /** @param {() => void} [callback] */
  authorizeSocket(callback) {
    // TODO type
    ...
    this.socket.emit(
      "authorize",
      auth_data,
      /** @param {{ read?: boolean }} data */ (data) => {
        if (!data) {
          console.log("Crashed!");
          this.crashed();
        }

        if (!data.read || !this.hasRequiredPermission()) {
          this.hashauth.requestAuthentication(() => {
            this.hashauth.updateSocketAuth();
  ...
  subscribeForAlarms() {
    // TODO type
    if (!this.alarmSocket) return;

    const auth_data = {
      secret: this.authorized?.token ? null : this.hashauth.hash(),
      jwtToken: this.authorized?.token,
    };

    this.alarmSocket.emit(
      "subscribe",
      auth_data,
      /** @param {{ read?: boolean; success?: boolean }} data */ (data) => {
        if (!data) {
          console.log("Crashed!");
          this.crashed();
        }
        ...
        if (!data.success) {
          if (
            ...
            this.settings?.authenticationPromptOnLoad
          ) {
            return this.hashauth.requestAuthentication(() =>
              this.hashauth.updateSocketAuth()
            );
          }
        }
  ...
  hasRequiredPermission() {
    if (this.requiredPermission) {
      if (this.hashauth && this.hashauth.isAuthenticated()) {
        return true;
      } else {
        return this.authorized?.check(this.requiredPermission);
      }
    } else {
      return true;
    }
  ...
  /** @protected */
  isAlarmForHigh() {
    // With predicted alarms, latestSGV may still be in target so to see if the
    // alarm is for a HIGH we can only check if it's >= the bottom of the target
    return !!(
      this.latestSGV &&
      this.settings && // TODO types
      this.latestSGV.mgdl >= this.settings.thresholds.bgTargetBottom
    );
  }
  ...
  isAlarmForLow() {
    // With predicted alarms, latestSGV may still be in target so to see if the
    // alarm is for a LOW we can only check if it's <= the top of the target
    return !!(
      this.latestSGV &&
      this.settings && // TODO types
      this.latestSGV.mgdl <= this.settings.thresholds.bgTargetTop
    );
  }
  ...
  isAlarm() {
    ...
    return (
      (this.isAlarmForHigh() && this.settings.alarmUrgentHigh) ||
      (this.isAlarmForLow() && this.settings.alarmUrgentLow)
    );
  }
  ...
  updateAdminMenu() {
    ...
    $(".foodcontrol").toggle(this.settings.enable.includes("food"));
    // hide cob control if not enabled
    $(".cobcontrol").toggle(this.settings.enable.includes("cob"));
  }
  ...
  prepareEntries() {
    ...
    const rawbgs =
      !this.ddata.cal || !this.rawbg.isEnabled(sbx)
        ? []
        : this.ddata.sgvs
            .map((entry) => {
              // TODO types
              if (!this.ddata) return null;

              const showRawbgs = this.rawbg.showRawBGs(
                entry.mgdl,
                entry.noise,
                this.ddata.cal,
                sbx
              );
              const rawbgValue = showRawbgs
                ? this.rawbg.calc(entry, this.ddata.cal, sbx)
                : 0;

              if (rawbgValue <= 0) return null;

              return {
                mills: entry.mills - 2000,
                mgdl: rawbgValue,
                color: "white",
                type: /** @type {const} */ ("rawbg"),
              };
            })
            .filter((e) => e !== null);

    /** @type {import("../types").Sgv[]} */
    const sgvs = this.ddata.sgvs.map((entry) => ({
      mills: entry.mills,
      mgdl: entry.mgdl,
      direction: entry.direction,
      color: this.sgvToColor(entry.mgdl),
      type: /** @type {const} */ ("sgv"),
      noise: entry.noise,
    ...
    const mbgs = this.ddata.mbgs.map((entry) => ({
      mills: entry.mills,
      mgdl: entry.mgdl,
      color: "red",
      type: /** @type {const} */ ("mbg"),
    ...
    const tooOld = (this.now ?? Date.now()) - times.hours(48).msecs;
    this.entries = [...rawbgs, ...sgvs, ...mbgs]
      .filter((e) => e.mills > tooOld)
      .map((e) => ({
        ...e,
        ...(e.mgdl < 39 && { color: "transparent" }),
      }))
      .sort((a, b) => a.mills - b.mills);
  }

  /**
   * @param {import("./receiveddata").Received} received
   * @param {boolean} [headless]
   */
  dataUpdate(received, headless) {
    // TODO type
    if (
      !this.now ||
      !this.ddata ||
    ...
    console.info("got dataUpdate", new Date(this.now ?? Date.now()));

    this.dataLastUpdated = Date.now();

    receiveDData(received, this.ddata, this.settings);

    // Resend new treatments to profile
    this.profilefunctions.updateTreatments(
      this.ddata.profileTreatments,
      this.ddata.tempbasalTreatments,
      this.ddata.combobolusTreatments
    );

    if (received.profiles) {
      this.profilefunctions.loadData(received.profiles);
    }

    if (this.ddata.sgvs) {
      // TODO change the next line so that it uses the prediction if the signal gets lost (max 1/2 hr)
      this.ctx.data.lastUpdated = this.dataLastUpdated;
      this.latestSGV = this.ddata.sgvs.at(-1);
    }

    this.ddata.inRetroMode = false;
    this.ddata.profile = this.profilefunctions;

    this.nowSBX = sandbox.clientInit(
      this.ctx,
      this.dataLastUpdated,
      this.ddata
    );
    ...
    //all enabled plugins get a chance to set properties, even if they aren't shown
    this.plugins.setProperties(this.nowSBX);
    ...
    if (!this.isInitialData) {
      ...
      this.chart = require("./chart")(this, d3, $);
      this.chart.update(true);
      this.brushed();
      this.chart.update(false);
    } else if (!this.inRetroMode()) {
      this.brushed();
      this.chart?.update(false);
    } else {
      this.chart?.updateContext();
    }
  }
}
...
//         // detect browser language
//         var lang =
//           Storages.localStorage.get("language") ||
...
//     settings: client.settings,
//     pluginBase: client.plugins.base(
//       majorPills,
//       minorPills,
//       statusPills,
//       bgStatus,
...
//     client.authorized.shiros = _.map(
//       client.authorized.permissionGroups,
//       function toShiro(group) {
...
//     var treatmentCreateAllowed = client.authorized
//       ? client.authorized.check("api:treatments:create")
//       : isAuthenticated;
//     var treatmentUpdateAllowed = client.authorized
//       ? client.authorized.check("api:treatments:update")
//       : isAuthenticated;
...
//   client.boluscalc = require("./boluscalc")(client, global.$ || $);
...
//       return [
//         client.entryToDate(client.entries[0]),
//         client.entryToDate(client.entries[client.entries.length - 1]),
//       ];
//     } else {
//       return [
//         new Date(client.now - times.hours(history).msecs),
//         new Date(client.now),
//       ];
//     }
//   };
...
//     var bottomOfPrimary = primary.offset()
//       ? primary.offset().top + primary.height()
//       : 0;
//     var bottomOfMinorPills = minorPills.offset()
//       ? minorPills.offset().top + minorPills.height()
//       : 0;
//     var bottomOfStatusPills = statusPills.offset()
//       ? statusPills.offset().top + statusPills.height()
//       : 0;
...
//           title =
//             s(scaleBg(currentMgdl)) +
//             s(deltaDisplay) +
//             s(client.direction.info(client.latestSGV).label) +
//             title;
//         }
//       }
//     }
...
//     var result = {
//       inProgress: currentAnnouncement
//         ? Date.now() - currentAnnouncement.received < times.mins(5).msecs
//         : false,
//     };

//     if (result.inProgress) {
//       var message =
//         currentAnnouncement.message.length > 1
...
//     if (
//       !brushedRange ||
//       brushExtent[1].getTime() - brushExtent[0].getTime() !==
//         client.focusRangeMS
//     ) {
//       // ensure that brush updating is with the time range
//       if (
...
//       var reallyCurrentAndNotAlarming =
//         isCurrent && !inRetroMode() && !alarmingNow();
...
//       if (focusPoint) {
//         if (
//           brushExtent[1].getTime() - focusPoint.mills >
//           times.mins(15).msecs
//         ) {
//           clearCurrentSGV();
//         } else {
...
//         color = "yellow";
//       } else if (
//         sgv >= client.settings.thresholds.bgTargetBottom &&
//         sgv <= client.settings.thresholds.bgTargetTop &&
//         client.settings.theme === "colors"
//       ) {
//         color = "#4cff00";
...
//       notify &&
//       notify.isAnnouncement &&
//       notify.message &&
//       notify.message.length > 1;
...
//     container
//       .addClass("alarming")
//       .addClass(file === urgentAlarmSound ? "urgent" : "warning");

//     var silenceBtn = $("#silenceBtn");
//     silenceBtn.empty();

//     _.each(
//       client.settings.snoozeMinsForAlarmEvent(notify),
//       function eachOption(mins) {
//         var snoozeOption = $(
//           '<li><a data-snooze-time="' +
...
//         times.mins(15).msecs -
//         Math.abs(client.authorized.iat * 1000 - client.authorized.lat);
...
//       if (
//         opacity.current !== opacity.NIGHT &&
//         (dateTime.getHours() > 21 || dateTime.getHours() < 7)
//       ) {
...
//     client.now = Date.now();
//     $("#currentTime")
//       .text(formatTime(new Date(client.now), true))
//       .css("text-decoration", "");
//   }
...
//     return (
//       (client.settings.alarmTimeagoWarn && status === "warn") ||
...
//   client.alarmSocket = alarmSocket = io.connect("/alarm", {
//     multiplex: true,
//     transports: ["polling"],
//   });
...
//     if (
//       client.retro.loadedMills > 0 &&
//       Date.now() - client.retro.loadedMills > times.mins(5).msecs
//     ) {
//       resetRetro();
//       console.info("Cleared retro data to free memory");
//     }
...
//     alarmSocket.emit("subscribe", auth_data, function subscribeCallback(data) {
//       if (!data) {
//         console.log("Crashed!");
//         client.crashed();
//       }
...
//       console.log("Subscribed for alarms", data);
//       var shouldAuthenticationPromptOnLoad =
...
//         return (
//           client.authorized &&
//           client.authorized.check(client.requiredPermission)
//         );
//       }
//     } else {
//       return true;
...
//     return (
//       client.latestSGV &&
//       client.latestSGV.mgdl >= client.settings.thresholds.bgTargetBottom
//     );
//   }
...
//     return (
//       client.latestSGV &&
//       client.latestSGV.mgdl <= client.settings.thresholds.bgTargetTop
//     );
//   }
...
//     console.info("alarm received from server");
//     var enabled =
//       (isAlarmForHigh() && client.settings.alarmHigh) ||
//       (isAlarmForLow() && client.settings.alarmLow);
//     if (enabled) {
//       console.log("Alarm raised!");
...
//     console.info("urgent alarm received from server");
//     var enabled =
//       (isAlarmForHigh() && client.settings.alarmUrgentHigh) ||
//       (isAlarmForLow() && client.settings.alarmUrgentLow);
//     if (enabled) {
//       console.log("Urgent alarm raised!");
...
//       temp1 = client.ddata.sgvs
//         .map(function (entry) {
//           var rawbgValue = client.rawbg.showRawBGs(
//             entry.mgdl,
//             entry.noise,
//             client.ddata.cal,
//             sbx
...
//     client.entries = client.entries.concat(
//       client.ddata.mbgs.map(function (obj) {
//         return {
//           mills: obj.mills,
//           mgdl: obj.mgdl,
//           color: "red",
...
//     client.nowSBX = sandbox.clientInit(client.ctx, lastUpdated, client.ddata);
