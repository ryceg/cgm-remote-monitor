...
client.render = function render() {
  ...
  if (!rec) {
    if (!$errorMessage.length) {
      $inner.after(
        '<div id="errorMessage" title="No data found in DB">-?-</div>',
      );
    } else {
      $errorMessage.show();
    }
  ...
  if (face === "clock-color") {
    face =
      "c" +
      (window.serverSettings.settings.showClockLastTime ? "y" : "n") +
      "13-sg35-" +
      (window.serverSettings.settings.showClockDelta ? "dt14-" : "") +
      "nl-ar25-nl-ag6";
  } else if (face === "clock") {
    face = "bn0-sg40";
  ...
  for (let param in faceParams) {
    if (param === "0") {
      ...
      staleMinutes =
        faceParam.substr(2, 2) - 0 >= 0 ? faceParam.substr(2, 2) : 13; // threshold value (0=never)
    } else if (!clockCreated) {
      /* eslint-disable-next-line security/detect-object-injection */ // verified false positive
      let faceParam = faceParams[param];
      let div =
        '<div class="' +
        faceParam.substr(0, 2) +
        '"' +
        (faceParam.substr(2, 2) - 0 > 0
          ? ' style="' +
            (faceParam.substr(0, 2) === "ar" ? "height" : "font-size") +
            ":" +
            faceParam.substr(2, 2) +
            'vmin"'
          : "") +
        "></div>";
      $inner.append(div);
  ...
  let arrow = $('<img alt="arrow">').attr(
    "src",
    "/images/" +
      (!rec.direction || rec.direction === "NOT COMPUTABLE"
        ? "NONE"
        : rec.direction) +
      ".svg",
  );
  ...
  if (thresholdReached) {
    ...
    $(".ar")
      .css("filter", bgColor ? "brightness(100%)" : "brightness(50%)")
      .html(arrow);
  }
...
function updateClock() {
  let timeDivisor = parseInt(
    client.settings.timeFormat ? client.settings.timeFormat : 12,
    10,
  );
  let today = new Date(),
