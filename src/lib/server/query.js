...
function enforceDateFilter (query, opts) {
  ...
  // Only iterate keys if dateValue is an object (i.e., contains operators like $gte)
  if (dateValue && typeof dateValue === 'object' && !Array.isArray(dateValue)) {
    Object.keys(dateValue).forEach(function(key) {
      ...
      if (isNaN(dateString)) {
        // Ensure we are dealing with a string before trying to replace
        if (typeof dateString === 'string') {
          dateString = dateString.replace(' ', '+'); // some clients don't excape the plus
        }

        const validDate = dayjs(dateString).isValid();
        ...
        const d = dayjs.parseZone(dateString);
        dateValue[key] = d.toISOString();
      }
    });
  }

  // If no specific date filter on opts.dateField (usually 'date')
  // AND no top-level dateString filter
  // AND no specific dateString filter within find object (e.g. query.find.dateString)
  // AND noDateFilter is not explicitly true
  // THEN add a default date range filter.
  if (!dateValue && !query.dateString && !(query.find && query.find.dateString) && true !== opts.noDateFilter) {
    var minDate = Date.now( ) - opts.deltaAgo;
    query[opts.dateField] = {
      $gte: opts.useEpoch ? minDate : dayjs(minDate).toISOString()
    };
  }
}
