...
function storage (collection, ctx) {
  ...
  function create (statuses, fn) {
    ...
    for (let i = 0; i < statuses.length; i++) {
      ...
      let d;
      if (dayjs(obj.created_at).isValid()) {
        d = dayjs.parseZone(obj.created_at);
      } else {
        d = dayjs.utc();
      }
      obj.created_at = d.toISOString(); // d is already UTC
      obj.utcOffset = d.utcOffset(); // Should be 0 as d is in UTC mode
