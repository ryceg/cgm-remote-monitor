...
function boot (env, language) {
  ...
  function setupStorage (ctx, next) {
    ...
    try {
      if (env.storageURI && env.storageURI.startsWith('openaps://')) {
  ...
  function setupInternals (ctx, next) {
    ...
    // Initialize profilefunctions with context
    // ctx.profilefunctions = require('../profilefunctions')(null, ctx);
  ...
  function setupListeners (ctx, next) {
    ...
    var updateData = debounce(function debouncedUpdateData ( ) {
