...
function prepareData(obj) {
  ...
  const d = dayjs(obj.created_at).isValid() ? dayjs(obj.created_at) : dayjs();
  obj.created_at = d.utc().toISOString(); // Ensure created_at is always UTC
  ...
  const offset = d.utcOffset(); // Get the original offset from the input string
  ...
  if (obj.eventTime) {
    const eventTimeObj = dayjs(obj.eventTime);
    if (eventTimeObj.isValid()) {
      results.created_at = eventTimeObj.utc().toISOString(); // Ensure eventTime is also UTC
      obj.utcOffset = eventTimeObj.utcOffset(); // Update offset if eventTime is used
      results.offset = obj.utcOffset;
    }
  }
