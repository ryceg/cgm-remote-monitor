...
loopalyzer.getSGVs = function(datastorage, daysToShow) {
  ...
  daysToShow.forEach(function(day) {
    ...
    var fromDate = dayjs(day);
    var toDate = dayjs(day);
    fromDate = fromDate.set({ 'hours': 0, 'minutes': 0, 'seconds': 0, 'milliseconds': 0 });
    toDate = toDate.set({ 'hours': 0, 'minutes': 5, 'seconds': 0, 'milliseconds': 0 }); // toDate is 5 mins ahead
    for (let i = 0; i < 288; i++) {
      var found = false;
      data.some(function(record) {
        var recDate = dayjs(record.displayTime);
      ...
      fromDate = fromDate.add(5, 'minutes');
      toDate = toDate.add(5, 'minutes');
    }
...
loopalyzer.getBasals = function(datastorage, daysToShow, profile) {
  ...
  daysToShow.forEach(function(day) {
    var dayStart = dayjs(day).startOf('day');
    var dayEnd = dayjs(day).endOf('day');
    var basals = [];
    for (var i = 0; i < 288; i++) basals.push(NaN); // Clear the basals by filling with NaNs

    var index = 0;

    for (var dt = dayStart; dt < dayEnd; dt = dt.add(5, 'minutes')) {
  ...
  daysToShow.forEach(function(day) {
    ...
    if (iobStatusAvailable) {
      ...
      iobArray.forEach(function(entry) {
        var index = Math.floor(dayjs(entry.mills).diff(dayStart, 'minutes') / 5);
...
loopalyzer.fillNanWithTreatments = function(array, treatments) {
  treatments.forEach(function(treatment) {
    var dayStart = dayjs(treatment.date).startOf('day');
    var minutesAfterMidnight = dayjs(treatment.date).diff(dayStart, 'minutes');
...
loopalyzer.getCarbTreatments = function(datastorage, daysToShow) {
  ...
  var startDate = dayjs(daysToShow[0]);
  var endDate = dayjs(daysToShow[daysToShow.length - 1]).add(1, 'days');

  datastorage.treatments.filter(function(treatment) { return treatment.carbs && treatment.carbs > 0 }).forEach(function(treatment) {
    if (dayjs(treatment.created_at).isBetween(startDate, endDate)) {
...
loopalyzer.getAllPredictionsForADay = function(datastorage, day) {
  ...
  for (var i = datastorage.devicestatus.length - 1; i >= 0; i--) {
    if (datastorage.devicestatus[i].loop && datastorage.devicestatus[i].loop.predicted) {
      ...
      if (dayjs(predicted.startDate).isSame(dayStart, 'day'))
...
loopalyzer.findPredicted = function(predictions, timestamp, offset) {
  ...
  if (offset && offset < 0) { // If offset is negative, start searching from first prediction going forward
    for (let i = 0; i < predictions.length; i++) {
      if (predictions[i] && predictions[i].startDate && dayjs(predictions[i].startDate) <= ts) {
        predicted = i;
      }
    }
...
loopalyzer.getPredictions = function(datastorage, daysToShow, client) {
  ...
  var bins = [];
  var date = dayjs().set({ 'hours': 0, 'minutes': 0, 'seconds': 0, 'milliseconds': 0 });
  for (var i = 0; i < 288; i++) {
    ...
    if (predictions.length > 0 && treatmentTimestamps.length > 0) {
      ...
      for (var treatmentsIndex = 0; treatmentsIndex < treatmentTimestamps.length; treatmentsIndex++) {
        ...
        if (predictedIndex != null) {
          ...
          var end = dayjs(day).endOf('day'); // Default to stop and end of the day
          if (truncatePredictions) {
            if (predictedOffset >= 0) {
              ...
              if (treatmentsIndex < treatmentTimestamps.length - 1) {
                end = dayjs(treatmentTimestamps[treatmentsIndex + 1]);
              }
            } else {
              // And if we are looking backward then we want to stop at "this" treatment
              end = dayjs(treatmentTimestamps[treatmentsIndex]);
            }
          }
          for (var entryIndex in entry.values) {
            if (!d.isAfter(end)) {
              var dayStart = dayjs(d).startOf('day');
              var minutesAfterMidnight = dayjs(d).diff(dayStart, 'minutes');
              var index = Math.floor(minutesAfterMidnight / 5);
...
loopalyzer.getEmptyBins = function() {
  var bins = [];
  var todayStart = dayjs().startOf('day');
  var todayEnd = dayjs().endOf('day');
  for (var dt = todayStart; dt < todayEnd; dt = dt.add(5, 'minutes')) {
    bins.push([dt.toDate(), []]);
  }
  return bins;
}
...
loopalyzer.renderProfilesTable = function(datastoreProfiles, daysToShow, client) {
    ...
    if (store) {
      for (var key in store) {
        ...
        if (Object.prototype.hasOwnProperty.call(store, key)) {
          ...
          if ((newEntry.basal || newEntry.carbratio || newEntry.sens) && dayjs(entry.startDate).isBefore(endOfLastDay))
  ...
  profilesArray2.forEach(function(entry) {
    if (laDebug)
      console.log(entry.startDate + ' isBefore ' + beginningOfFirstDay + ' = ' + dayjs(entry.startDate).isBefore(beginningOfFirstDay));
    if (dayjs(entry.startDate).isBefore(beginningOfFirstDay))
      latestProfile = entry;
  });
  ...
  profilesArray2.forEach(function(entry) {
    if (laDebug)
      console.log(entry.startDate + ' isAfter ' + beginningOfFirstDay + ' = ' + dayjs(entry.startDate).isAfter(beginningOfFirstDay));
    if (dayjs(entry.startDate).isAfter(beginningOfFirstDay))
...
loopalyzer.report = function(datastorage, sorteddaystoshow, options) {
  ...
  var firstDay = dayjs(daysToShow[0]);
  var lastDay = dayjs(daysToShow[daysToShow.length - 1]);
...
loopalyzer.generateReport = function(datastorage, daysToShow, options) {
  ...
  var dateInfo = dayjs(daysToShow[0]).format('ddd MMM D'); // .split(',')[0];
  if (daysToShow.length > 1) dateInfo += ' - ' + dayjs(daysToShow[daysToShow.length - 1]).format('ddd MMM D'); // .split(',')[0];
  ...
  if ($("#rp_loopalyzertimeshift").is(":checked") && daysToShow.length > 1) {
    ...
    if (t2 > t1) {
      ...
      daysToShow.forEach(function(day, dayIndex) {
        var timeShiftBegin = dayjs(day).set({ 'hours': h1, 'minutes': m1, 'seconds': 0 });
        var timeShiftEnd = dayjs(day).set({ 'hours': h2, 'minutes': m2, 'seconds': 0 });

        var found = false;
        carbTreatments.forEach(function(entry) {
          if (!found && entry.amount >= mealMinCarbs) {
            var date = dayjs(entry.date);
            if ((date.isSame(timeShiftBegin, 'minute') || date.isAfter(timeShiftBegin, 'minute')) &&
              ...
              var startOfDay = dayjs(entry.date).set({ 'hours': 0, 'minutes': 0, 'seconds': 0 });
      ...
      timeShiftStartTime = dayjs(todayJSON);
      timeShiftStartTime.minutes(averageMinutesAfterMidnight);
      timeShiftStopTime = dayjs(todayJSON);
  ...
  insulinTreatments.forEach(function(treatment) {
    var startDate = dayjs(treatment.date);
    var endDate = dayjs(treatment.date);
    startDate.set(todayJSON);
    endDate.set(todayJSON);
    endDate = endDate.add(5, 'minutes');
