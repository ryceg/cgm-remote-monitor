...
function init (env, callback) {
  ...
  if (!env.storageURI || typeof env.storageURI !== 'string') {
  ...
  function loadData (path) {
    ...
    try {
      ...
      if (Array.isArray(inputData)) {
        ...
        inputData.forEach(addId);
      } else if (inputData && typeof inputData === 'object' && inputData !== null && Object.keys(inputData).length > 0) {
  ...
  function reportAsCollection (name) {
    var data = { };
    var input = config?.collections?.[name]?.input;
    if (Array.isArray(input)) {
      //console.info('>>>input is an array', input);
      data[name] = input.map(loadData).flat();
    } else {
    ...
    var wrapper = {
      ...
      , find: function find (query) {
        function deepCloneAndTransform(obj) {
          if (!obj || typeof obj !== 'object') return obj;
          if (Array.isArray(obj)) {
            return obj.map(deepCloneAndTransform);
          }
          const result = {};
          for (const key in obj) {
            const value = obj[key];
            if (value && typeof value === 'object' && value !== null && '$exists' in value) {
              result[key] = { $exists: true };
            } else {
              result[key] = deepCloneAndTransform(value);
            }
          }
          return result;
        }
        query = deepCloneAndTransform(query);
      ...
      , toArray: function toArray(callback) {
        collection.find(wrapper.findQuery).toArray(function intercept (err, results) {
          ...
          if (wrapper.sortQuery) {
            var field = Object.keys(wrapper.sortQuery).pop();
          ...
          if (wrapper.limitCount !== null && typeof wrapper.limitCount === 'number' && !isNaN(wrapper.limitCount)) {
            //console.info('>>>limit count', wrapper.limitCount);
            results = results.slice(0, wrapper.limitCount);
          }
  ...
  try {
    ...
    config = deepMerge({}, customConfig, config);
