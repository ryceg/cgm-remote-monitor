...
function configure (app, wares, ctx, env) {
    ...
    function sync (data, next) {
      ...
      if (env.settings.deNormalizeDates) {
        if (data.dateString && Object.prototype.hasOwnProperty.call(data, 'utcOffset')) {
          const d = dayjs(data.dateString).utcOffset(data.utcOffset);
  ...
  function ifModifiedSinceCTX (req, res, next) {
    ...
    var lastEntry = ctx.ddata.sgvs.length > 0 ? ctx.ddata.sgvs[ctx.ddata.sgvs.length - 1] : null;
    var lastEntryDate = null;

    if (lastEntry != null) {
      lastEntryDate = new Date(ctx.ddata.sgvs[ctx.ddata.sgvs.length - 1].mills);
  ...
  function format_entries (req, res) {
    ...
    var lastEntry = res.entries.length > 0 ? res.entries[0] : null;
    var lastEntryDate = null;
  ...
  function query_models (req, res, next) {
    ...
    if (typeQuery) {
      inMemoryCollection = ctx.cache.entries.filter(function checkType (object) {
    ...
    if (inMemoryPossible && query.count <= inMemoryCollection.length) {
      res.entries = JSON.parse(JSON.stringify(inMemoryCollection.slice(0, query.count)));
   ...
   */  function prep_storage (req, res, next) {
    if (req.params.storage && ['entries', 'treatments', 'devicestatus'].includes(req.params.storage)) {
