...
function init (env, ctx) {
  ...
  function extractJWTfromRequest (req) {
    ...
    if (!token) {
      ...
      if (!accessToken && req.body) {
        if (Array.isArray(req.body) && req.body.length > 0 && req.body[0].token) {
  ...
  function apiSecretFromRequest (req) {
    ...
    if (!secret && req.body) {
      ...
      if (Array.isArray(req.body) && req.body.length > 0 && req.body[0].secret) {
  ...
  authorization.checkMultiple = function checkMultiple (permission, shiros) {
    var found = shiros && Array.isArray(shiros) ?
      shiros.find(function checkEach (shiro) {
  ...
  authorization.isPermitted = function isPermitted (permission) {
    authorization.seenPermissions = [...new Set([...authorization.seenPermissions, permission])].sort();
  ...
  authorization.authorize = function authorize (accessToken) {
    ...
    if (subject) {
      ...
      var roles = subject.roles ? [...new Set(subject.roles.concat(defaultRoles))] : defaultRoles;
    authorized = {
        token
        , sub: subject.name
        , permissionGroups: roles.map(storage.roleToPermissions)
        , iat: decoded.iat
        , exp: decoded.exp
      };
    }
