...
describe("Database Size", function () {
  var dataInRange = {
    dbstats: {
      dataSize: 1024 * 1024 * 137,
      indexSize: 1024 * 1024 * 48,
      fileSize: 1024 * 1024 * 256,
    },
  };
  var dataWarn = {
    dbstats: {
      dataSize: 1024 * 1024 * 250,
      indexSize: 1024 * 1024 * 100,
      fileSize: 1024 * 1024 * 360,
    },
  };
  var dataUrgent = {
    dbstats: {
      dataSize: 1024 * 1024 * 300,
      indexSize: 1024 * 1024 * 150,
      fileSize: 1024 * 1024 * 496,
    },
  };
  ...
  it("display database size warning notiffication", function (done) {
    ...
    notif.message.should.equal(
      "Database size is 350 MiB out of 496 MiB. Please backup and clean up database!",
    );
    done();
  });
  ...
  it("display database size urgent notiffication", function (done) {
    ...
    notif.message.should.equal(
      "Database size is 450 MiB out of 496 MiB. Please backup and clean up database!",
    );
    done();
  });
  ...
  it("should handle virtAsst requests", function (done) {
    ...
    dbsize.virtAsst.intentHandlers[0].intentHandler(
      function next(title, response) {
        title.should.equal("Database file size");
        response.should.equal(
          "450 MiB. That is 90% of available database space.",
        );
