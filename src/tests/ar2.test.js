...
describe("ar2", function () {
  ...
  it("should trigger a warning alarm by interpolating when more than 5mins apart", function (done) {
    ...
    ctx.ddata.sgvs = [
      { mgdl: 120, mills: before - SIX_MINS },
      { mgdl: 85, mills: now },
    ];
  ...
  it("should handle virtAsst requests", function (done) {
    ...
    ar2.virtAsst.intentHandlers[0].intentHandler(
      function next(title, response) {
        title.should.equal("AR2 Forecast");
        response.should.equal(
          "According to the AR2 forecast you are expected to be between 109 and 120 over the next in 30 minutes",
        );
        done();
      },
      [],
