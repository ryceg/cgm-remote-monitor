...
describe("timeago", function () {
  ...
  function freshSBX() {
    ...
    var sbx = require("../lib/sandbox")()
      .serverInit(env, ctx)
      .withExtendedSettings(timeago);
    return sbx;
  }
  ...
  it("Not trigger an alarm with future data", function (done) {
    ctx.notifications.initRequests();
    ctx.ddata.sgvs = [
      { mills: Date.now() + times.mins(15).msecs, mgdl: 100, type: "sgv" },
    ];
  ...
  it("should trigger a warning when data older than 15m", function (done) {
    ctx.notifications.initRequests();
    ctx.ddata.sgvs = [
      { mills: Date.now() - times.mins(16).msecs, mgdl: 100, type: "sgv" },
    ];
  ...
  it("should trigger an urgent alarm when data older than 30m", function (done) {
    ctx.notifications.initRequests();
    ctx.ddata.sgvs = [
      { mills: Date.now() - times.mins(31).msecs, mgdl: 100, type: "sgv" },
    ];
  ...
  it("calc timeago displays", function () {
    ...
    should.deepEqual(timeago.calcDisplay({ mills: now }, now), {
      value: 1,
      label: "min ago",
      shortLabel: "m",
    });

    should.deepEqual(timeago.calcDisplay({ mills: now - 1 }, now), {
      value: 1,
      label: "min ago",
      shortLabel: "m",
    });
